...
def compute_rs_rows(prices: dict, xu100: pd.Series) -> list:
    """prices: {sym: DataFrame(Close,Volume)} · xu100: Close serisi.
    ...
    for sym, df in prices.items():
        ...
        for k, n in LOOKBACKS.items():
            r = _ret(close, n)
            ir = idx_ret.get(k)
            exc[k] = (r - ir) if (r is not None and ir is not None) else None
        adv = None
        try:
            tl = (close * vol).dropna()
            if len(tl) >= 5:
...
def _fetch_xu100():
    ...
    for tk in XU100_TICKERS:
        try:
            df = yf.download(tk, period=FETCH_PERIOD, interval="1d",
...
def run_rs_scan() -> dict:
    ...
    try:
        xu = _fetch_xu100()
        if xu is None:
            return {"status": "XU100 çekilemedi"}
        ...
        if yf is not None and syms:
            for i in range(0, len(syms), BATCH):
                ...
                tickers = [s + ".IS" for s in chunk]
                try:
                    bulk = yf.download(tickers, period=FETCH_PERIOD, interval="1d",
                                       progress=False, auto_adjust=True,
                                       group_by="ticker", threads=True)
                    for s in chunk:
                        tk = s + ".IS"
                        try:
                            sub = bulk[tk] if isinstance(bulk.columns, pd.MultiIndex) else bulk
                            if sub is not None and not sub.empty:
                                prices[s] = sub[["Close", "Volume"]].copy()
                        except Exception:
                            pass
                except Exception:
                    pass
                _scanning["progress"] = min(i + BATCH, len(syms))

...
def _analyze_one(symbol: str, df: pd.DataFrame):
    ...
    try:
        ...
        # vakalarını ele.
        # ============================================================
        BULL_BARS    = 5      # Son 5 günün HEPSİ ST > LSMA olmalı
        MIN_MARGIN   = 0.005  # Son barda en az %0.5 margin
        FRESH_DAYS   = 4      # Son bar tarihi 4 günden eski ise atla

        if len(st) < BULL_BARS or len(lsma) < BULL_BARS:
            return None
        bull_window = (st.iloc[-BULL_BARS:] - lsma.iloc[-BULL_BARS:])
        ...
        last_lsma = float(lsma.iloc[-1])
        if last_lsma <= 0:
            return None
        if (last_st - last_lsma) / last_lsma < MIN_MARGIN:
            return None

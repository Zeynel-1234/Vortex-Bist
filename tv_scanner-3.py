...
def fetch_all_timeframes(symbol: str) -> Dict:
    ...
    NOT: Bazı enstrümanlar (girişim sermayesi/fon tipi yatırım ortaklıkları,
    örn. ISGSY) 'turkey' market taramasının varsayılan kapsamına girmiyor.
    O yüzden boş dönerse set_tickers('BIST:SEMBOL') ile DOĞRUDAN çekiyoruz —
    bu yöntem market kapsamından bağımsızdır ve tip filtrelemez.
    """
    try:
        cols = _build_columns()
        df = None

        # 1) Standart yöntem: turkey market + isim eşleşmesi
        try:
            _c, df = (Query()
                      .select(*cols)
                      .set_markets('turkey')
        ...
        # 2) FALLBACK: tam ticker ile doğrudan (kapsam/tip bağımsız)
        if df is None or len(df) == 0:
            try:
                _c, df = (Query()
                          .select(*cols)
                          .set_tickers(f'BIST:{symbol}')
                          .get_scanner_data())
            except Exception:
                df = None

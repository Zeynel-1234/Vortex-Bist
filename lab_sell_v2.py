...
def evaluate_v2_signals(df: pd.DataFrame, signal: pd.Series,
    ...
    success_count = int((arr_d >= 15.0).sum())  # %15+ düşüş
    big_success = int((arr_d >= 30.0).sum())     # %30+ düşüş
    ...
    pct_lag_0_2 = float((arr_l <= 2).sum()) / n_sig * 100   # 0-2 bar = mükemmel
    pct_lag_0_5 = float((arr_l <= 5).sum()) / n_sig * 100   # 0-5 bar = iyi
    
    # Composite quality (peak proximity'yi yüksek ağırlıkla içerir)
    norm_drop = min(avg_drop / 40.0 * 100, 100)
    norm_reverse = min(avg_reverse / 20.0 * 100, 100)
    proximity_score = pct_lag_0_2  # 0-100, doğrudan
    
    quality = (
        success_rate * 0.25 +
        norm_drop * 0.25 +
        proximity_score * 0.30 +    # ← peak proximity en yüksek ağırlık
        min(n_sig / 10 * 100, 100) * 0.10 -
        norm_reverse * 0.10
    )
...
def build_sell_v2_dna(df: pd.DataFrame, symbol: str = '') -> Dict:
    ...
    # Hangisi daha iyi (KESIN'in yeterli sinyali varsa onu seç)
    ...
    if combined_quality >= 50 and chosen_test['n_signals'] >= 2 and not overfit:
        status = 'OK'
        reason = (f"v2 {chosen_mode} sat motoru hazır · "
                  f"kalite {combined_quality:.1f}/100 · "
                  f"peak_lag {chosen_test['avg_peak_lag']:.1f} bar · "
                  f"%{chosen_test['pct_at_peak_0_2']:.0f}'i 0-2 bar mesafede")
    elif chosen_test['n_signals'] >= 1:
        status = 'ZAYIF'
        reason = f"Yetersiz sinyal veya kalite ({combined_quality:.1f}, n={chosen_test['n_signals']})"
    else:
        status = 'FAIL'
        reason = "Test periyodunda v2 sinyali tetiklenmedi"

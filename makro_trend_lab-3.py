    ...
    try:
        ...
        for i, sym in enumerate(syms):
            ...
            try:
                ...
                taranan += 1
                v = verdict_last(s)
                if v["deception_index"] is not None and v["deception_index"] >= DI_MAX:
                    yaniltma += 1
                if v["verdict"].startswith("GÜÇLÜ AL") or v["verdict"] == "İZLE":
                    idx = np.where(s["SINYAL"].values)[0]
                    rows.append({
                        "sembol": sym, "verdict": v["verdict"],
            ...
            if (i + 1) % 25 == 0:
                gc.collect()
        gc.collect()
        rows.sort(key=lambda r: (r["verdict"].startswith("GÜÇLÜ AL"),
                                 r["skor"] or 0), reverse=True)
        _scan_state.update({

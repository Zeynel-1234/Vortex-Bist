...
def install_hakem(app, fetch_ohlc: Callable) -> None:
    ...
    @app.get("/hakem/karne")
    def hakem_karne():
        return _state.get("sonuc") or SON_KARNE

    @app.get("/hakem/status")
    def hakem_status():
        if _state["sonuc"] is None:          # restart sonrası kayıttan yükle
            try:
                h = MT._scan_state.get("hakem")
                if h:
                    _state["sonuc"] = h.get("sonuc")
                    _state["updated"] = h.get("updated")
            except Exception:
                pass
    ...
    def hakem_run(symbols: str = Query("")):
        ...
        def _go():
            try:
                ...
                try:    # KALICILIK: makro'nun /tmp+Gist durumuna bindir (restart'ı atlatır)
                    MT._scan_state["hakem"] = {"updated": _state["updated"],
                                               "sonuc": _state["sonuc"]}
                    MT._save_scan()
                except Exception:
                    pass

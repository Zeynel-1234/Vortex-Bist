        ...
        try:
            ...
            if gid:
                ...
                if fobj and fobj.get("content"):
                    parsed = json.loads(fobj["content"])
                    # Yeni gist içeriği "{}" olabilir → yapı eksikse boş şablon ver
                    if isinstance(parsed, dict) and "cumulative" in parsed:
                        return parsed
                    return _empty_data()
                return _empty_data()
...
function openStock(sym){ window.location.href = '/?sym=' + encodeURIComponent(sym); }
...
function render(d){
  ...
  if(d.today_picks && d.today_picks[0] && d.today_picks[0].length){
    ...
    for(i=0;i<ps.length;i++){H+='<span class="tag" onclick="openStock(\''+ps[i]+'\')" style="cursor:pointer;background:rgba(34,197,94,.14);color:#22c55e;border:1px solid rgba(34,197,94,.3)">'+ps[i]+' ›</span>';}
    H+='<div class="muted" style="margin-top:6px">En çok sinyalin birleştiği hisseler — en yüksek olasılıklı bahisler (kanıt biriktikçe doğrula). Tıkla → NVS detayı.</div></div>';
  }
  ...
  if(m.days_evaluated>0){
    ...
    if(m.stocks && m.stocks.length){
      ...
      for(i=0;i<m.stocks.length;i++){r=m.stocks[i];
        H+='<tr onclick="openStock(\''+r.sym+'\')" style="cursor:pointer"><td class="l lbl">'+r.sym+' ›</td><td class="l"><span class="pill">'+r.best_signal+'</span></td>'+
          '<td class="'+hrClass(r.best_hit_rate,base)+'">'+pct(r.best_hit_rate)+'</td>'+
          '<td class="muted">'+r.best_picks+'</td></tr>';
      }
      H+='</table><div class="muted" style="margin-top:6px">Her hissenin geçmişte en çok %5+ getirdiği sinyal. Min '+'3 gözlem. Tıkla → NVS detayı.</div></div>';
    }
  }

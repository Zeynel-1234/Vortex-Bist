...
# ════════════════════════════════════════════════════════════════
W_CS    = 0.22   # Açık gök / yeni zirve (en güçlü yapısal ayraç)
W_SQ    = 0.20   # Sıkışma (depolanmış enerji)
W_RS    = 0.18   # Göreceli güç (liderlik)
W_RVOL  = 0.15   # Hacim patlaması (katılım)
W_CLV   = 0.13   # Kapanış gücü (talep vs dağıtım)
W_BASE  = 0.12   # Taban sıkılığı (VCP)
W_EXT   = 0.20   # Uzama CEZASI (çıkarılır)
...
def score_symbol(symbol):
    ...
    positive = (W_CS * cs_s + W_SQ * sq_s + W_RS * rs_s +
...
def _render_compare(items):
    head = ("<tr><th style='text-align:left;padding:6px'>Hisse</th>"
            ...
            "<th style='padding:6px'>AçıkGök</th><th style='padding:6px'>Sıkışma</th>"
    ...
    for d in items:
        if d.get("hata"):
            body += "<tr><td style='padding:6px'>%s</td><td colspan=9 style='color:#ef4444;padding:6px'>%s</td></tr>" % (d.get("symbol"), d["hata"])
            continue
        m = {c["ad"].split(" ")[0]: c["skor"] for c in d["bilesenler"]}
        cells = [m.get("Açık", 0), m.get("Sıkışma", 0), m.get("Göreceli", 0),
...
def compare(symbols: str = "CELHA,GSDDE", fmt: str = "html"):
    syms = [s.strip().upper() for s in symbols.split(",") if s.strip()][:10]
    items = [score_symbol(s) for s in syms]
    if fmt == "json":
...
def get_patlama(symbol: str, fmt: str = "html"):
    d = score_symbol(symbol)
    if fmt == "json":

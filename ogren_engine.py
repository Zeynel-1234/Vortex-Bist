...
def compute_weights(tracker_data: dict) -> dict:
    ...
    Beklenen tracker_data yapısı (signal_tracker /tracker/data):
      {"signals": {SINYAL: {"n": int, "hits": int, "hit_rate": float}, ...},
       "base_rate": float}   # base_rate yoksa sinyallerden tahmin edilir
    ...
    Sonra ağırlıklar 0..1'e normalize edilir (görece kıyas için).
    """
    sigs = (tracker_data or {}).get("signals", {}) or {}
    if not sigs:
        return {"weights": {}, "base_rate": None,
                "note": "tracker verisi yok — önce /tracker/run birikmeli"}

    # base rate: verilmişse al, yoksa tüm sinyallerin havuzundan tahmin et
    base = (tracker_data or {}).get("base_rate")
    if base is None:
        tot_h = sum(float(s.get("hits", 0)) for s in sigs.values())
        tot_n = sum(float(s.get("n", 0)) for s in sigs.values())
    ...
    for name, s in sigs.items():
        ...
        if hits == 0 and s.get("hit_rate") is not None and n > 0:
            hits = int(round(float(s["hit_rate"]) * n))
...
def _active_signals_for_symbol(sym, scan_row, crossover_syms):
    """Bir hissenin hangi sinyalleri AKTİF — /scan satırı + kesişim listesinden."""
    out = []
    if scan_row:

...
ONKAYIT = {
    ...
    "kalicilik_aralik": [30, 120],  # vuruş+30 ile vuruş+120 arası HER kapanış...
...
def etiketle(df: pd.DataFrame,
    ...
    for grup in gruplar:
        ...
        ar_bas, ar_son = ONKAYIT["kalicilik_aralik"]
        j_son = j_hit + ar_son
        if j_son >= n or (sma_n - 1) > j_hit:
            sinif = "BELIRSIZ"   # sınamak için veri henüz yok/yetersiz
        else:
            # KESİNTİSİZ kalıcılık: vuruş+30'dan vuruş+120'ye kadar
            # HER kapanış eşiğin üstünde kalmalı. Nokta kontrolü
            # kandırılabilir (aralarda dip), film kandırılamaz.
            esik_f = c[t0] * (1.0 + ONKAYIT["kalicilik_min"])
            kalici = bool(np.min(c[j_hit + ar_bas: j_son + 1]) >= esik_f)
            seri = 0
            en_uzun = 0

...
def _autostart_scan():
    """Açılışta RS boşsa BİR KEZ otomatik tarar. Render restart'larında
    (her deploy + boşta uyuma) RS verisi uçtuğunda kendini iyileştirir;
    böylece Gist veya 18:30 zamanlayıcısı olmadan da panel RS gösterir.
    RS_GIST_ID tanımlıysa load_state Gist'ten dolu gelir → tarama atlanır."""
    import time as _t
    _t.sleep(90)   # uvicorn'un tam ayağa kalkması + /scan hazır olması için bekle
    try:
        st = load_state()
        rows = st.get("rows") or []
        if rows:
            print("[rs_engine] startup: RS dolu (n=%s) → otomatik tarama atlandı" % len(rows))
            return
        print("[rs_engine] startup: RS boş → otomatik /rs/refresh başlatılıyor")
        res = run_rs_scan()
        print("[rs_engine] startup tarama bitti ·", res)
    except Exception as e:
        print("[rs_engine] startup tarama hatası:", str(e)[:160])


def install_rs(app) -> None:
    app.include_router(rs_router)
    # Kendini iyileştiren açılış taraması (RS_AUTOSTART_OFF=1 ile kapatılabilir)
    off = os.environ.get("RS_AUTOSTART_OFF", "").strip() in ("1", "true", "True", "yes")
    if not off and not _autostart_done["on"]:
        _autostart_done["on"] = True
        threading.Thread(target=_autostart_scan, daemon=True).start()

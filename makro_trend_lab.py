...
_MAKRO_GIST_ID = os.environ.get("MAKRO_GIST_ID", "").strip()
_GH_TOKEN = os.environ.get("GITHUB_TOKEN", "").strip()
_GIST_FILE = "makro_scan.json"


def _gist_hdr():
    return {"Authorization": "token " + _GH_TOKEN,
            "Accept": "application/vnd.github+json", "User-Agent": "makro"}


def _gist_load():
    if not (_GH_TOKEN and _MAKRO_GIST_ID):
        return None
    try:
        import requests
        r = requests.get("https://api.github.com/gists/" + _MAKRO_GIST_ID,
                         headers=_gist_hdr(), timeout=30)
        if r.status_code == 200:
            f = r.json().get("files", {}).get(_GIST_FILE)
...
def _gist_save():
    global _MAKRO_GIST_ID
    if not _GH_TOKEN:
        return
    try:
        import requests
        payload = {"files": {_GIST_FILE: {"content": json.dumps(_scan_state, ensure_ascii=False)}}}
        if _MAKRO_GIST_ID:
            requests.patch("https://api.github.com/gists/" + _MAKRO_GIST_ID,
                           headers=_gist_hdr(), json=payload, timeout=30)
        else:
            payload["description"] = "Vortex-BIST makro+kesisim gunluk tarama"
            payload["public"] = False
            r = requests.post("https://api.github.com/gists",
                              headers=_gist_hdr(), json=payload, timeout=30)
            if r.status_code in (200, 201):
                _MAKRO_GIST_ID = r.json().get("id")
                print("[makro] MAKRO_GIST_ID=" + str(_MAKRO_GIST_ID) +
                      "  ← Render env'e ekle (günlük kalıcılık restart'ları atlatsın)")
    except Exception:
        pass


def _load_scan():
    """Önce /tmp (aynı instance), yoksa Gist (restart/uyku sonrası)."""
    global _scan_state
    try:
...
def run_universe_scan(fetch_ohlc: Callable) -> Dict:
    ...
    try:
        ...
        # Kesişim tarayıcının CANLI önbelleğini günlük snapshot olarak sakla
        # (restart sonrası kesişim belleği boşalsa bile birleşik liste dolu kalır)
        try:
            import crossover_scanner as _cs
            with _cs._lock:
                _cd = list(_cs._cache.get("data") or [])
            if _cd:
                _scan_state["kesisim"] = {
                    (r.get("symbol") or "").upper(): {
                        "kat": r.get("category"), "ks": r.get("cross_date"),
                        "gun": r.get("days_since_cross"),
                        "pct": r.get("pct_change"),
                        "gunluk": r.get("daily_change_pct")}
                    for r in _cd if r.get("symbol")}
        except Exception:
            pass
        _scan_state.update({
            "updated": _dt.datetime.now().isoformat(timespec="seconds"),
            "gun": _dt.date.today().isoformat(),
...
def install_makro(app, fetch_ohlc: Callable) -> None:
    ...
    def makro_scan_run(force: int = Query(0, ge=0, le=1)):
        today = _dt.date.today().isoformat()
        if (not force and _scan_state.get("gun") == today
                and _scan_state.get("updated")):
            return {"status": "bugun_guncel",
                    "not": "Bugünün taraması zaten kayıtlı. Zorlamak için force=1."}
    ...
    @app.get("/makro/birlesik")
    def makro_birlesik():
        """MAKRO + KESİŞİM birleşik liste. Kesişim: önce CANLI önbellek,
        boşsa (restart sonrası) günlük snapshot."""
        kes = {}
        try:
            import crossover_scanner as _cs
            with _cs._lock:
            ...
            for r in _cd:
                ...
                if s:
                    kes[s] = {"kat": r.get("category"), "ks": r.get("cross_date"),
                              "gun": r.get("days_since_cross"),
                              "pct": r.get("pct_change"),
                              "gunluk": r.get("daily_change_pct")}
        except Exception:
            pass
        if not kes:
            kes = _scan_state.get("kesisim") or {}
        mak = {r["sembol"]: r for r in (_scan_state.get("rows") or [])}
        _t = {"SINYAL": 3, "IZLE": 2, "YAKIN": 1}
        _k = {"YENİ": 3, "ORTA": 2, "YÜKSEK": 1}   # YENİ = kesişime en yakın = en aksiyone edilebilir
        rows = []
        for s in sorted(set(list(mak.keys()) + list(kes.keys()))):
            m, k = mak.get(s), kes.get(s)
            w = (_t.get((m or {}).get("tip"), 0) * 10
                 + ((m or {}).get("skor") or 0) / 10.0
                 + (_k.get((k or {}).get("kat"), 0) if k else 0)
                 + (50 if (m and k) else 0))       # çift sinyal en üste
            rows.append({"sembol": s, "makro": m, "kesisim": k, "_w": w})
        rows.sort(key=lambda r: r["_w"], reverse=True)
        for r in rows:
            r.pop("_w", None)
        return {"gun": _scan_state.get("gun"), "updated": _scan_state.get("updated"),
                "makro_aday": len(mak), "kesisim_aday": len(kes),
                "cift_sinyal": sum(1 for r in rows if r["makro"] and r["kesisim"]),
                "taranan": _scan_state.get("taranan"),
                "yaniltma_sayisi": _scan_state.get("yaniltma"),
                "rows": rows[:150]}

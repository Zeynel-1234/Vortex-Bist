...
# ═══════════════════════════════════════════════════════════════
# EK MOTORLAR · SAVUNMACI KURULUM
#   Herhangi bir modül eksik/bozuk olsa bile uygulama ÇÖKMEZ; o özellik
#   devre dışı kalır ve log'a net hata yazılır. (Deploy'un status 1 ile
#   komple ölmesini önler — eksik dosyayı log'dan görürsün.)
# ═══════════════════════════════════════════════════════════════
try:
    from bvi_engine import install_bvi
    install_bvi(app)                       # Kapı 6 — Kırılım Doğrulama (BVI)
except Exception as _e:
    print("[main] BVI (bvi_engine) yuklenemedi:", repr(_e))

try:
    from auto_scheduler import start_scheduler
    start_scheduler()                      # her iş günü 18:30 TR otonom /scan+ÖĞREN+/rs/refresh
except Exception as _e:
    print("[main] auto_scheduler yuklenemedi:", repr(_e))

try:
    from sat_combo_lab import install_sat_combo
    install_sat_combo(app, fetch_ohlc)     # /satlab/* — Stoch RSI × Chande Kroll tepe motoru
except Exception as _e:
    print("[main] sat_combo_lab yuklenemedi:", repr(_e))

try:
    from factor_engine import install_factor
    install_factor(app, fetch_ohlc)        # /factor/* — 1-3 aylık çok-faktörlü kompozit (Kapı 7)
except Exception as _e:
    print("[main] factor_engine yuklenemedi:", repr(_e))

try:
    from makro_trend_lab import install_makro
    install_makro(app, fetch_ohlc)         # /makro/{symbol} · /makro/backtest/run — Deception Index + trend başlangıcı
except Exception as _e:

...
def _comp_range(high, low, close):
    """Son barın mum boyu / ATR — genişleme = ateşleme işareti."""
    try:
        atr = _wilder_atr(high, low, close, ATR_LEN)
        a = float(atr.iloc[-1])
        if len(close) >= 2:
            pc = float(close.iloc[-2])
            tr = max(float(high.iloc[-1]) - float(low.iloc[-1]),
                     abs(float(high.iloc[-1]) - pc), abs(float(low.iloc[-1]) - pc))
        else:
            tr = float(high.iloc[-1]) - float(low.iloc[-1])
        ratio = tr / a if a > 0 else 0.0
        return _lin(ratio, 1.0, 2.5), round(ratio, 2)
    except Exception:
        return 50.0, None


def _breakout_info(high, close):
    """Kapanış, son 52H zirvenin üstünde mi (yeni zirve) + zirveye yakınlık oranı."""
    try:
        h = pd.to_numeric(high, errors="coerce").dropna()
        c = float(close.iloc[-1])
        prior = h.iloc[-(HIGH_WINDOW + 1):-1] if len(h) > HIGH_WINDOW else h.iloc[:-1]
        hh = float(prior.max()) if len(prior) else c
        cs = c / hh if hh > 0 else 1.0
        return cs, (c >= hh), hh
    except Exception:
...
# ════════════════════════════════════════════════════════════════
# ANA SKORLAYICI — v1.2: 2 FAZLI (KURULUM + ATEŞLEME)
# ════════════════════════════════════════════════════════════════
def score_symbol(symbol, asof_back=0):
    ...
    # Bileşenler
    sq_s, sq_v, sq_t = _comp_squeeze(close)        # KURULUM
    ba_s, ba_v, ba_t = _comp_base(high, low, close)  # KURULUM
    rv_s, rv_v, rv_t = _comp_rvol(volume)          # ATEŞLEME (hacim)
    rng_s, rng_v = _comp_range(high, low, close)   # ATEŞLEME (mum boyu)
    cs, is_newhigh, hh = _breakout_info(high, close)
    brk_s = 100.0 if is_newhigh else _lin(cs, 0.75, 1.0, 0, 92)  # ATEŞLEME (kırılım)
    prox_s = _lin(cs, 0.78, 1.0)                   # KURULUM: dirence yakınlık
    er_s, er_v, er_t = _comp_efficiency(close)     # bağlam
    rs_s, rs_v, rs_t = _comp_rs(close, xu)         # bağlam
    cl_s, cl_v, cl_t = _comp_clv(high, low, close)  # ateşleme teyidi
    ex_s, ex_v, ex_t = _comp_ext(high, low, close)  # ceza

    # ── FAZ SKORLARI ──────────────────────────────────────────────
    setup = _clamp(0.45 * sq_s + 0.25 * ba_s + 0.30 * prox_s, 0, 100)
    ignition = _clamp(0.45 * rv_s + 0.30 * brk_s + 0.25 * rng_s, 0, 100)
    context = _clamp((rs_s + er_s + cl_s) / 3.0, 0, 100)

    # ── TETİK AKTİF Mİ? (hacim şart) ──────────────────────────────
    rvol = rv_v if rv_v is not None else 0
    trigger = (rvol >= 1.8) and (is_newhigh or (rng_v or 0) >= 1.5)

    if trigger:
        base = 0.70 * ignition + 0.30 * context
        patlama = _clamp(base - W_EXT * ex_s, 0, 100)
        if ex_s >= 60:
            durum = "🚀 TETİK ama ÇOK UZAMIŞ — kovalama riski"; vcolor = "#e8b84b"
        elif patlama >= 62:
            durum = "🚀 TETİK AKTİF — TAVAN ADAYI (AL)"; vcolor = "#22c55e"
        else:
            durum = "TETİK zayıf — temkinli"; vcolor = "#e8b84b"
    elif setup >= 62:
        patlama = _clamp(min(72, 0.60 * setup + 0.40 * context), 0, 72)
        durum = "🟡 YÜKLENMİŞ YAY — İZLE (hacimli tetik bekle)"; vcolor = "#e8b84b"
    else:
        patlama = _clamp(0.40 * setup + 0.30 * ignition + 0.30 * context, 0, 55)
        durum = "ZAYIF / CHOP — PAS"; vcolor = "#ef4444"

    comps = [
        {"ad": "Açık Gök (kırılım)", "skor": round(brk_s, 1), "faz": "ateşleme",
         "deger": round(cs, 3), "aciklama": ("YENİ ZİRVE — vakum" if is_newhigh
                                             else "52H zirvenin %%%d'i" % int(cs * 100))},
        {"ad": "Hacim (RVOL)", "skor": round(rv_s, 1), "faz": "ateşleme",
         "deger": rv_v, "aciklama": rv_t},
        {"ad": "Mum Genişlemesi", "skor": round(rng_s, 1), "faz": "ateşleme",
         "deger": rng_v, "aciklama": "son mum %.2f×ATR" % (rng_v or 0)},
        {"ad": "Sıkışma (squeeze)", "skor": round(sq_s, 1), "faz": "kurulum",
         "deger": sq_v, "aciklama": sq_t},
        {"ad": "Taban Sıkılığı (VCP)", "skor": round(ba_s, 1), "faz": "kurulum",
         "deger": ba_v, "aciklama": ba_t},
        {"ad": "Dirence Yakınlık", "skor": round(prox_s, 1), "faz": "kurulum",
         "deger": round(cs, 3), "aciklama": "zirveye yakınlık"},
        {"ad": "Göreceli Güç (RS)", "skor": round(rs_s, 1), "faz": "bağlam",
         "deger": rs_v, "aciklama": rs_t},
        {"ad": "Verim (ER)", "skor": round(er_s, 1), "faz": "bağlam",
         "deger": er_v, "aciklama": er_t},
        {"ad": "Kapanış Gücü (CLV)", "skor": round(cl_s, 1), "faz": "bağlam",
         "deger": cl_v, "aciklama": cl_t},
        {"ad": "Uzama CEZASI (EXT)", "skor": round(ex_s, 1), "faz": "ceza",
         "deger": ex_v, "aciklama": ex_t},
    ]
    ...
    return {
        ...
        "tetik_aktif": bool(trigger),
        "fiyat": round(float(close.iloc[-1]), 2),
        "asof_back": asof_back,
        "bilesenler": comps,
        "likidite_TL_haftalik": _liquidity(close, volume),
        "veri_hafta": len(df),
        "not": "TAVAN = KURULUM (sıkışma) + ATEŞLEME (hacim≥~2× + kırılım). Hacim yoksa tetik yok. Olasılıksal; garanti değil.",
    }
...
def _render_html(d):
    if d.get("hata"):
        return ("<html><body style='background:#0a0d14;color:#e7eefc;font-family:monospace;padding:20px'>"
                ...
                "<p style='color:#4b5e78'>Örnek: /patlama/SANEL · /patlama/compare?symbols=SANEL,ESCOM</p>"
                "</body></html>") % (d.get("symbol", "?"), d["hata"])
    fazlar = [("ateşleme", "🔥 ATEŞLEME (şimdi ateşliyor mu?)", "#ff6b3d"),
              ("kurulum", "🧨 KURULUM (yüklenmiş yay mı?)", "#60a5fa"),
              ("bağlam", "📐 BAĞLAM", "#9aa7bd"),
              ("ceza", "⚠️ CEZA", "#ef4444")]
    rows = ""
    for fkey, ftitle, fcol in fazlar:
        grp = [c for c in d["bilesenler"] if c.get("faz") == fkey]
        if not grp:
            continue
        rows += "<div style='margin:14px 0 4px;font-size:11px;color:%s;font-weight:700'>%s</div>" % (fcol, ftitle)
        for c in grp:
            sc = c["skor"]
            ...
            rows += ("<div style='margin:8px 0'>"
                     ...
                     "<span style='color:%s;font-weight:700'>%.0f</span></div>"
                     "%s"
                     "<div style='font-size:10px;color:#4b5e78;margin-top:2px'>%s · değer: %s</div>"
                     "</div>") % (c["ad"], col, c["skor"], _bar(c["skor"], col),
                                  c["aciklama"], str(c["deger"]))
    liq = d.get("likidite_TL_haftalik")
    liq_s = "{:,.0f} TL/hafta".format(liq) if liq else "—"
    setup = d.get("setup", 0); ign = d.get("ignition", 0)
    scol = "#22c55e" if setup >= 62 else "#e8b84b" if setup >= 45 else "#ef4444"
    icol = "#22c55e" if ign >= 62 else "#e8b84b" if ign >= 45 else "#ef4444"
    phase_box = ("<div style='display:flex;gap:10px;margin:12px 0'>"
                 "<div style='flex:1;text-align:center;padding:12px;border:1px solid %s;border-radius:10px'>"
                 "<div style='font-size:10px;color:#7a9ab8'>🧨 KURULUM</div>"
                 "<div style='font-size:30px;font-weight:800;color:%s;line-height:1.1'>%s</div></div>"
                 "<div style='flex:1;text-align:center;padding:12px;border:1px solid %s;border-radius:10px'>"
                 "<div style='font-size:10px;color:#7a9ab8'>🔥 ATEŞLEME</div>"
                 "<div style='font-size:30px;font-weight:800;color:%s;line-height:1.1'>%s</div></div>"
                 "</div>") % (scol, scol, setup, icol, icol, ign)
    asof = ("  ·  %d hafta öncesine sarıldı" % d["asof_back"]) if d.get("asof_back") else ""
...
<h2 style='margin:0'>%s</h2><span style='color:#4b5e78'>₺%s%s</span></div>
...
%s
<div style='font-size:11px;color:#7a9ab8;margin-bottom:2px'>BİLEŞENLER</div>
%s
...
<a style='color:#00d4ff' href='/patlama/compare?symbols=SANEL,DGGYO,AVGYO,ESCOM'>↔ SANEL/DGGYO/AVGYO/ESCOM karşılaştır</a></div>
</body></html>""") % (d["symbol"], d["symbol"], d["fiyat"], asof, d["renk"], d["renk"],
                       d["patlama"], d["renk"], d["durum"], phase_box, rows, liq_s,
                       d["veri_hafta"], d["not"])


def _render_compare(items):
    head = ("<tr><th style='text-align:left;padding:6px'>Hisse</th>"
            "<th style='padding:6px'>PATLAMA</th><th style='padding:6px'>Kurulum</th>"
            "<th style='padding:6px'>Ateşleme</th><th style='padding:6px'>AçıkGök</th>"
            "<th style='padding:6px'>RVOL</th><th style='padding:6px'>Tetik</th>"
            "<th style='text-align:left;padding:6px'>Durum</th></tr>")
    body = ""
    for d in items:
        if d.get("hata"):
            body += "<tr><td style='padding:6px'>%s</td><td colspan=7 style='color:#ef4444;padding:6px'>%s</td></tr>" % (d.get("symbol"), d["hata"])
            continue
        m = {c["ad"].split(" ")[0]: c["skor"] for c in d["bilesenler"]}
        def cc(v):
            return "#22c55e" if v >= 62 else "#e8b84b" if v >= 45 else "#ef4444"
        tetik = "🚀" if d.get("tetik_aktif") else "—"
        body += ("<tr><td style='padding:6px;font-weight:700'>%s</td>"
                 ...
                 "<td style='text-align:center;padding:6px;color:%s'>%s</td>"
                 "<td style='text-align:center;padding:6px;color:%s'>%s</td>"
                 "<td style='text-align:center;padding:6px;color:%s'>%.0f</td>"
                 "<td style='text-align:center;padding:6px;color:%s'>%.0f</td>"
                 "<td style='text-align:center;padding:6px'>%s</td>"
                 "<td style='padding:6px;color:%s;font-size:11px'>%s</td></tr>") % (
            d["symbol"], d["renk"], d["patlama"],
            cc(d.get("setup", 0)), d.get("setup", 0),
            cc(d.get("ignition", 0)), d.get("ignition", 0),
            cc(m.get("Açık", 0)), m.get("Açık", 0),
            cc(m.get("Hacim", 0)), m.get("Hacim", 0),
            tetik, d["renk"], d.get("durum", ""))
    return ("""<html><head><meta name=viewport content='width=device-width,initial-scale=1'>
...
<div style='overflow-x:auto'><table style='border-collapse:collapse;font-size:12px;min-width:600px'>
%s%s</table></div>
<p style='color:#4b5e78;font-size:11px;margin-top:12px'>🚀 Tetik = hacimli kırılım (AL anı). Kurulum yüksek + tetik yok = yüklenmiş yay (izle). Olasılıksal; garanti değil.</p>
</body></html>""") % (head, body)

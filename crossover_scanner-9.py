...
def _analyze_one(symbol: str, df: pd.DataFrame):
    ...
    try:
        ...
        # ============================================================
        # KOŞUL (HAFTALIK): ST yeşil çizgi, EMA kırmızıya DEĞDİĞİNDE
        # veya ÜZERİNDE olduğu TÜM durumlarda sinyal.
        # "Değme" = ST, EMA'nın %0.5 altına kadar yakınsa da sayılır.
        # ============================================================
        last_st  = float(st.iloc[-1])
        last_ema = float(ema.iloc[-1])
        if last_ema <= 0:
            return None
        # KESİN KOŞUL: ST (yeşil), EMA'ya (kırmızı) DEĞDİ (eşit) VEYA ÜSTÜNDE.
        # ST, EMA'nın ALTINDAYSA → kesinlikle ELE (sinyal yok).
        if last_st < last_ema:
            return None
...
def api_check(symbol: str):
    ...
    try:
        ...
        return {
            ...
            "SON_BAR_SINYAL_VERIR_MI": last.get("ST>=EMA"),
            "son_8_hafta": rows,
            "NOT": "ST>=EMA True ise sinyal verir. TV'deki yeşil(ST)/kırmızı(EMA) ile karşılaştır.",
        }
    except Exception as e:

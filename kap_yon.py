...
ONKAYIT = {"baslangic": "2010-01-01", "pencere_gun": 5,
           ...
           "cag_siniri": "2018-01-01", "surum": "kap_yon_v1.1"}
...
_BASLIK_TEMIZ = re.compile(
    r"Pay\s+Alım\s+Satım\s+Bildirimi|Alım\s+Satım\s+Bildirimi|"
    r"Pay\s+Alım\s+Satım|Alım\s*[-/]\s*Satım")


def _yon_say(metin: str) -> Dict:
    """SPK formu 'Alım/Satım' der; başlık kalıpları nötr olduğundan
    sayım öncesi temizlenir. Alış/Satış varyantları da kapsanır."""
    m = _BASLIK_TEMIZ.sub(" ", metin)
    a = len(re.findall(r"\b(?:Alım|Alış|ALIM|ALIŞ)\b", m))
    s = len(re.findall(r"\b(?:Satım|Satış|SATIM|SATIŞ)\b", m))
    ...
    def kapyon_ornek(id: str = Query("1633516")):
        ...
        try:
            ...
            if p["pdf_mi"] and PdfReader is not None:
                ...
                sayfalar = []
                for sf in pdf.pages[:12]:
                    t_ = sf.extract_text() or ""
                    sayfalar.append(t_)
                    metin += t_
                p.update(_yon_say(metin))
                p["sayfa_sayisi"] = len(pdf.pages)
                p["parca"] = metin[:250]
                p["son_parca"] = (sayfalar[-1] if sayfalar else "")[:400]
            rapor["pdf"] = p
        except Exception as e:
            rapor["pdf"] = {"hata": repr(e)[:90]}
        # EK KEŞFİ: asıl form kapakta değil; ekleri ve alt-formları ara
        ekler_rapor = {}
        # a) bildirim detay JSON'u (ekler listesi burada olabilir)
        for yol in ("/tr/api/notification/" + id,
                    "/tr/api/disclosure/notification/" + id,
                    "/tr/api/notification/detail/" + id):
            try:
                rr = ses.get(KAP + yol, headers=hg, timeout=40)
                if rr.status_code == 200:
                    m = _coz(rr.content or b"")
                    kayit = {"http": 200, "boyut": len(rr.content or b""),
                             **_yon_say(m)}
                    # ek dosya kimliklerini yakala
                    ekid = re.findall(r'"(?:attachmentId|fileId|ekId)"\s*:\s*"?(\d+)', m)
                    kayit["ek_kimlikleri"] = list(dict.fromkeys(ekid))[:8]
                    kayit["parca"] = m[:200]
                    ekler_rapor[yol] = kayit
                    break
                else:
                    ekler_rapor[yol] = {"http": rr.status_code}
            except Exception as e:
                ekler_rapor[yol] = {"hata": repr(e)[:70]}
            time.sleep(0.6)
        rapor["ek_kesfi"] = ekler_rapor

        # b) liste kaydının summary'sinde yön var mı? (en ucuz yol)
        try:
            import datetime as _dt
            # id'nin yayın tarihini bilmiyoruz; son 30 günü tarayıp bu id'yi bul
            bugun = _dt.date.today()
            r = ses.post(KAP + "/tr/api/disclosure/members/byCriteria",
                         json={"fromDate": str(bugun - _dt.timedelta(days=30)),
                               "toDate": str(bugun),
                               "mkkMemberOidList": [], "subjectList": []},
                         headers={"Referer": KAP + "/tr/bildirim-sorgu",
                                  "User-Agent": UA,
            ...
            bulunan = next((d for d in v
                            if str(d.get("disclosureIndex")) == id), None)
            if bulunan:
                sm = str(bulunan.get("summary") or "")
                rapor["liste_summary"] = {"metin": sm[:200], **_yon_say(sm),
                                          "attachmentCount":
                                          bulunan.get("attachmentCount")}
            else:
                rapor["liste_summary"] = {"not": "id son 30 gunde bulunamadi"}
        except Exception as e:
            rapor["liste_summary"] = {"hata": repr(e)[:70]}

        rapor["karar"] = _detay_yon(id)
        rapor["tani"] = ("ek_kesfi veya liste_summary'de alis/satis>0 olan "
                         "kaynak = gercek yon kapisi. Hepsi 0 ise yon ekli "
                         "alt-formda; ek_kimlikleri ile o forma inilir.")
        return rapor

...
FETCH_PERIOD  = "3y"        # 12-1 momentum + 52h yükseği için >=252 bar gerekir
FETCH_WORKERS = 6
...
def run_factor_scan(fetch_ohlc: Callable) -> Dict:
    ...
    try:
        ...
        with _lock:
            save_state(state)
        return {"status": "ok", "n": len(rows), "universe": len(syms),
...
def install_factor(app, fetch_ohlc: Callable) -> None:
    ...
    if not off and not _autostart_done["on"]:
        ...
        def _autostart():
            import time as _t
            _t.sleep(120)   # RS taramasından sonra başlasın; app tam ayakta olsun
            try:
                st = load_state()
                if st.get("rows"):

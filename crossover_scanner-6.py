"""
================================================================================
  Vortex-BIST · Supertrend × EMA(20) Crossover Scanner
  v1.1 — Gösterge güncellemesi: LSMA → EMA(20), Supertrend(25,1) → Supertrend(2,1)
        Tüm diğer mantık birebir korundu (BULL_BARS, MIN_MARGIN, kategoriler).

  Tarama Mantığı (günlük bar):
    - Yeşil çizgi  = Supertrend(ATR=2, factor=1.0)   ← TV: "Supertrend 2 1"
    - Sarı çizgi   = EMA(close, length=20)
    - Koşul        = Supertrend > EMA(20)  (yukarı kesişim aktif)
...
def _analyze_one(symbol: str, df: pd.DataFrame):
    ...
    try:
        ...
        ema = compute_ema(close, length=EMA_LENGTH)
...
<div class="foot">Vortex-BIST · Supertrend(2,1) × EMA(20) · v1.1</div>

"""
================================================================================
  Vortex-BIST · Supertrend(20,1) × EMA(20,high) HAFTALIK Crossover Scanner
  v2.0 — HAFTALIK + Supertrend(20,1) × EMA(high,20). Eski mantık korundu.
...
# ============================================================================
TIMEFRAME     = "1wk"   # HAFTALIK bar (TV: "H")
ST_ATR_LENGTH = 20      # Supertrend ATR period (TV: "Supertrend 20 1" → 20)
...
def _analyze_one(symbol: str, df: pd.DataFrame):
    ...
    try:
        ...
        if len(idx) < 30:                        # EMA20 + ST20 (haftalık) için yeterli geçmiş
            return None

        close = close.loc[idx]; high = high.loc[idx]; low = low.loc[idx]

        # ────────────────────────────────────────────────────────────
        # KRİTİK: İçinde bulunduğumuz HAFTA henüz KAPANMADI.
        # TV, kapanmamış haftada Supertrend'i çevirmez (yeşil çizgi yatay
        # kalır = son kapanan haftanın değeri). Ama yfinance yarım haftanın
        # close'unu verir; bu close ST'yi yukarı banda flip ettirip EMA'nın
        # üstüne çıkarabilir → SAHTE sinyal. Bu yüzden yarım haftayı atıp
        # SON KAPANAN haftalık bara bakıyoruz (TV ile birebir aynı).
        # ────────────────────────────────────────────────────────────
        import datetime as _dt
        try:
            lb = close.index[-1]
            lbd = lb.date() if hasattr(lb, "date") else _dt.date.fromisoformat(str(lb)[:10])
            if (_dt.date.today() - lbd).days <= 6:     # bu haftanın barı → yarım
        ...
        # ============================================================
        # KOŞUL (HAFTALIK, son KAPANAN bar):
        ...
        FRESH_DAYS = 20   # son KAPANAN haftalık bar ~7-14 gün eski olabilir
        try:
            import datetime as _dt
...
def run_full_scan():
    ...
    try:
        for batch_start in range(0, len(_symbols), BATCH):
            ...
            try:
                bulk = yf.download(
                    tickers, period="5y", interval=TIMEFRAME,
...
def api_check(symbol: str):
    ...
    try:
        df = yf.download(sym + ".IS", period="5y", interval=TIMEFRAME,
                         progress=False, auto_adjust=True)
        if df is None or df.empty:
        ...
        idx = close.index[close.notna() & high.notna() & low.notna()]
        close = close.loc[idx]; high = high.loc[idx]; low = low.loc[idx]
        # Yarım (kapanmamış) haftayı at — _analyze_one ile aynı
        dropped = None
        try:
            import datetime as _dt2
            lb = close.index[-1]
            lbd = lb.date() if hasattr(lb, "date") else _dt2.date.fromisoformat(str(lb)[:10])
            if (_dt2.date.today() - lbd).days <= 6:
                dropped = str(lb)[:10]
                close = close.iloc[:-1]; high = high.iloc[:-1]; low = low.iloc[:-1]
        except Exception:
            pass
        ...
        for i in range(max(0, len(close) - 8), len(close)):
            ...
            rows.append({
                ...
                "ST(20,1)": round(s, 4) if s is not None else None,
        ...
        return {
            "sembol": sym,
            "interval": TIMEFRAME,
            "atilan_yarim_hafta": dropped,
            "params": {"ST_ATR": ST_ATR_LENGTH, "ST_FACTOR": ST_FACTOR,
                       "EMA": EMA_LENGTH, "EMA_kaynak": EMA_SOURCE},
            "bar_sayisi": len(close),
            "SON_KAPANAN_HAFTA_SINYAL_VERIR_MI": last.get("SINYAL"),
            "son_8_hafta": rows,
            "NOT": "Kosul: ST YESIL (bullish) VE ST>=EMA. Yarim hafta atildi. TV'deki yesil(ST)/kirmizi(EMA) ile karsilastir.",
        }
    except Exception as e:
...
<div class="foot">Vortex-BIST · Supertrend(20,1) × EMA(high,20) · HAFTALIK · v2.0</div>

"""
================================================================================
  Vortex-BIST · Supertrend(10,1) × EMA(10,high) AYLIK Crossover Scanner
  v2.4 — AYLIK + Supertrend(10,1) × EMA(high,10). Mantık korundu, periyot AYLIK.

  Tarama Mantığı (HAFTALIK bar):
    - Yeşil çizgi  = Supertrend(ATR=25, factor=1.0)
    - Sarı çizgi   = EMA(source=high, length=10)    [AYLIK bar]
...
# ============================================================================
TIMEFRAME     = "1mo"   # AYLIK bar (TV: "M") — GÖSTERİM/etiket amaçlı
# ── VERİ ÇEKİMİ: yfinance'in "1mo" verisi GÜNCEL AYI geç oluşturuyor.
#    Bu yüzden GÜNLÜK çekip AYLIĞA KENDİMİZ resample ediyoruz (ay başı çapalı,
#    güncel YARIM ay DAHİL). Böylece TV'nin canlı AYLIK barıyla birebir uyumlu
#    oluyoruz ve kesişim oluştuğu AN (ay kapanmadan, günlük takipte) yakalanır.
FETCH_INTERVAL = "1d"    # gerçek indirilen interval (GÜNLÜK)
FETCH_PERIOD   = "10y"   # ~120 AYLIK bar → ST(10) Wilder ATR için fazlasıyla yeterli
ST_ATR_LENGTH = 10      # Supertrend ATR period (AYLIK — 50'den 10'a çekildi)
ST_FACTOR     = 1.0     # Supertrend multiplier (TV: "Supertrend 20 1" → 1)
EMA_LENGTH    = 10      # EMA period (AYLIK — 20'den 10'a çekildi)
EMA_SOURCE    = "high"  # EMA kaynağı: "high" (TV "EMA 20 high" ile aynı)
CACHE_TTL_SAAT = 6      # önbellek bu süreden eskiyse otomatik yeniden tara


def _to_monthly(df: pd.DataFrame) -> pd.DataFrame:
    """Günlük OHLC → AY BAŞI çapalı AYLIK bar (güncel YARIM ay DAHİL).
    TV'nin aylık barlarıyla (ayın 1'i) birebir uyumludur. Güncel (yarım) ay
    her gün güncellendiği için ST×EMA kesişimi OLUŞTUĞU GÜN sinyale döner.
    ...
    try:
        ...
        return df
    # Her günün ait olduğu ayın 1'i (ay başı çapa)
    month_start = d.index.to_period("M").to_timestamp()
    agg = {}
    ...
    if not agg:
        return df
    try:
        out = d.groupby(month_start).agg(**agg).sort_index()
...
def _to_weekly(df: pd.DataFrame) -> pd.DataFrame:
    ...
    (Korunuyor — artık tarama AYLIK kullanıyor, bu fonksiyon yedek/uyumluluk için.)
    """
    if df is None or len(df) == 0:
        return df
    d = df.copy()
...
def compute_ema(prices: pd.Series, length: int = 10) -> pd.Series:
...
def compute_supertrend(high: pd.Series, low: pd.Series, close: pd.Series,
                       length: int = 10, factor: float = 1.0):
...
def _analyze_one(symbol: str, df: pd.DataFrame):
    ...
    try:
        ...
        # GÜNLÜK veriyi AYLIĞA çevir (ay başı çapalı, güncel YARIM ay DAHİL)
        df = _to_monthly(df)
        if df is None or df.empty:
            return None
        ...
        if len(idx) < 30:                        # EMA10 + ST10 (AYLIK) için yeterli geçmiş
            return None
        ...
        # Veri tazeliği — AYLIK bar ay başına çapalı; güncel ay ~30 gün "eski"
        # görünebilir, bu yüzden toleransı aylık ölçeğe göre genişletiyoruz.
        FRESH_DAYS = 45
        try:
        ...
        # Aylık % değişim: son iki AYLIK bar'ın close farkı
        daily_pct = 0.0
        try:
            if len(close) >= 2:
...
# ============================================================================
def _cache_yas_dk():
    """Önbelleğin yaşı (dakika). Veri yoksa None."""
    lu = _cache.get("last_updated")
    if not lu:
        return None
    try:
        t = datetime.fromisoformat(lu)
        return max(0.0, (datetime.now() - t).total_seconds() / 60.0)
    except Exception:
        return None
...
def api_scan(background: BackgroundTasks, refresh: bool = False):
    ...
    # TTL: veri yoksa VEYA bayatladıysa (>CACHE_TTL_SAAT) otomatik tazele
    bayat = (yas is not None and yas > CACHE_TTL_SAAT * 60)
    if (refresh or empty or bayat) and not is_scanning:
        background.add_task(run_full_scan)

    with _lock:
        return {
            "last_updated": _cache["last_updated"],
            "cache_yas_dk": round(yas, 1) if yas is not None else None,
            "bayat": bool(bayat),
            "ttl_saat": CACHE_TTL_SAAT,
...
def api_check(symbol: str):
    ...
    try:
        ...
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)
        # GÜNLÜK → AYLIK (ay başı çapalı, güncel ay dahil) — _analyze_one ile aynı
        df = _to_monthly(df)
        if df is None or df.empty:
            return {"sembol": sym, "hata": "aylik veri yok"}
        close = pd.to_numeric(df["Close"], errors="coerce").dropna()
        high = pd.to_numeric(df["High"], errors="coerce").reindex(close.index)
        low = pd.to_numeric(df["Low"], errors="coerce").reindex(close.index)
        idx = close.index[close.notna() & high.notna() & low.notna()]
        ...
        for i in range(max(0, len(close) - 8), len(close)):
            ...
            rows.append({
                ...
                "EMA(high,10)": round(e, 4) if e is not None else None,
                "ST(10,1)": round(s, 4) if s is not None else None,
        ...
        return {
            "sembol": sym,
            "interval": TIMEFRAME,
            "guncel_ay_dahil": True,
            "params": {"ST_ATR": ST_ATR_LENGTH, "ST_FACTOR": ST_FACTOR,
                       "EMA": EMA_LENGTH, "EMA_kaynak": EMA_SOURCE},
            "bar_sayisi": len(close),
            "GUNCEL_AY_SINYAL_VERIR_MI": last.get("SINYAL"),
            "son_8_ay": rows,
            "NOT": "Kosul: ST YESIL (bullish) VE ST>=EMA. Guncel ay dahil (TV ile ayni). TV'deki yesil(ST)/kirmizi(EMA) ile karsilastir.",
        }
    except Exception as e:
...
<title>🟢 Supertrend × EMA10 · AYLIK · Vortex-BIST</title>
<style>
...
<h1>🟢 SUPERTREND × EMA10 · AYLIK
  <small>Yeşil (ST) sarı (EMA10) çizgiye değdi veya üstünde · AYLIK · kesişim sınıflandırması</small>
</h1>
...
<div class="foot">Vortex-BIST · Supertrend(10,1) × EMA(high,10) · AYLIK · v2.4</div>

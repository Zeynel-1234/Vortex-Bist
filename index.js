...
// ── TV SCANNER PROXY ────────────────────────────────────────────────────────
// Strateji: Önce direkt, sonra allorigins üzerinden GET (encoded), son çare corsproxy
app.post('/api/scanner', async (req, res) => {
  const body = typeof req.body === 'string' ? req.body : JSON.stringify(req.body);
  const errors = [];

  // 1. Direkt sunucudan çağır
  try {
    const data = await httpsPost('scanner.tradingview.com', '/turkey/scan', body);
  ...
  // 2. allorigins üzerinden (sunucudan)
  try {
    const encoded = encodeURIComponent('https://scanner.tradingview.com/turkey/scan');
    // allorigins GET ile çalışır, bu yüzden veriyi başka şekilde almamız gerekiyor
    // Bunun yerine scraper.info kullan
    const data = await httpsGet(`https://api.allorigins.win/raw?url=${encoded}`);
    // Bu GET isteği scanner'ı GET olarak çağırır, çalışmayabilir
    // Bunun yerine farklı yaklaşım: Yahoo Finance ile fiyat listesi
    throw new Error('allorigins GET scan çalışmaz');
  } catch (e) { errors.push('allorigins: ' + e.message); }

  // 3. Başarısız — Yahoo Finance'den fiyat listesi döndür
  console.log('TV Scanner başarısız, Yahoo Finance fallback:', errors);
  return res.json({ error: 'scanner_failed', errors });
});

// ── YAHOO FINANCE PROXY ─────────────────────────────────────────────────────
function yahooChart(sym, interval, range, res) {
  const urls = [
  ...
  let idx = 0;
  function tryNext() {
    if (idx >= urls.length) return res.status(502).json({ error: 'Yahoo ulaşılamadı' });
    httpsGet(urls[idx++])
      .then(d => { res.setHeader('Content-Type', 'application/json'); res.end(d); })
      .catch(() => tryNext());
  }
  tryNext();
}

// ── YAHOO MULTI-QUOTE (fiyat listesi için) ──────────────────────────────────
app.get('/api/quotes', async (req, res) => {
  const syms = (req.query.symbols || '').split(',').filter(Boolean);
  if (!syms.length) return res.json({ error: 'symbols gerekli' });
  const joined = syms.map(s => s.toUpperCase() + '.IS').join(',');
  try {
    const data = await httpsGet(`https://query1.finance.yahoo.com/v7/finance/quote?symbols=${joined}&fields=symbol,regularMarketPrice,regularMarketChangePercent,regularMarketChange`);
    res.setHeader('Content-Type', 'application/json');
    res.end(data);
  } catch (e) {
    try {
      const data = await httpsGet(`https://query2.finance.yahoo.com/v7/finance/quote?symbols=${joined}&fields=symbol,regularMarketPrice,regularMarketChangePercent`);
      res.setHeader('Content-Type', 'application/json');
      res.end(data);
    } catch (e2) {
...
app.get('/api/monthly/:sym', (req, res) => yahooChart(req.params.sym.toUpperCase(), '1mo', '3y', res));
app.get('/api/:sym', (req, res) => yahooChart(req.params.sym.toUpperCase(), req.query.tf || '1d', req.query.range || '1y', res));
app.get('/health', (req, res) => res.json({ ok: true, time: new Date().toISOString() }));
app.get('/', (req, res) => { res.setHeader('Content-Type', 'text/html; charset=utf-8'); res.send(HTML); });

app.listen(PORT, () => console.log('VORTEX BAŞLADI port=' + PORT));
...
#cmsg{position:absolute;inset:0;background:rgba(7,9,15,.96);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;z-index:10;font-size:12px;color:#4b5e78;padding:20px;text-align:center}
...
<div id="stbar">
  ...
  <span id="smsg" style="color:#e8b84b;">Başlatılıyor...</span>
  <button onclick="refreshAll()" style="margin-left:auto;background:transparent;border:1px solid #1a2030;color:#4b5e78;padding:2px 8px;border-radius:3px;cursor:pointer;font-size:9px;font-family:inherit;">↺ Yenile</button>
</div>
<div id="tabs">
...
<div id="tp-sinyal" class="pnl hid">
  ...
  <div id="couter">
    <div id="cmsg">
      ...
      <span id="cmt" style="color:#4b5e78;">Sol listeden hisse seç</span>
...
<div id="tp-fiyat" class="pnl">
  <div style="background:#0b1510;border-bottom:1px solid #1a3a1a;padding:5px 10px;font-size:10px;color:#22c55e;flex-shrink:0;display:flex;align-items:center;">
    ...
    <span id="src-lbl" style="font-size:8px;color:#2a3548;margin-left:6px;"></span>
...
// ═══════════════════════════════════════════════════════════════════
var SL=[
  "THYAO","AKBNK","GARAN","EREGL","ASELS","KCHOL","ISCTR","VAKBN",
  "HALKB","YKBNK","FROTO","TOASO","BIMAS","MGROS","ARCLK","TUPRS",
  "PETKM","SASA","EKGYO","TTKOM","TCELL","PGSUS","TAVHL","ENKAI",
  "KOZAL","KOZAA","AEFES","CCOLA","ULKER","LOGO","MAVI","OTKAR",
  "TTRAK","GUBRF","AKCNS","CIMSA","NUHCM","BRISA","KORDS","AYGAZ",
  "DOHOL","DOAS","SKBNK","ALBRK","QNBFB","TSKB","ENJSA","AKSEN",
  "AKENR","GWIND","ODAS","ZOREN","ASTOR","MPARK","TKFEN","ALKIM",
  "SISE","ANACM","A1CAP","AKGRT","AKSA","ALARK","ANSGR","ARENA",
  "ASUZU","BANVT","BERA","BFREN","BIZIM","BJKAS","BRSAN","BTCIM",
  "BUCIM","CRFSA","DARDL","DEVA","ECILC","ERCB","EUPWR","FENER",
  "GARFA","GESAN","GSDHO","GSRAY","HEKTS","HLGYO","HOROZ","IHLAS",
  "INDES","ISATR","ISGYO","IZMDC","KARSN","KARTN","KONTR","KONYA",
  "KRDMD","MUTLU","OYAKC","PAPIL","POLHO","PRKAB","RAYSG","RNSHO",
  "RYSAS","SELEC","SNGYO","TRGYO","TURSG","VESTL","VKGYO","YYLGD",
  "BAGFS","AGHOL","AHGAZ","KMPUR","DEVA","KIMMR"
];
// İsim haritası
var NAMES={THYAO:"THY",AKBNK:"Akbank",GARAN:"Garanti",EREGL:"Ereğli",ASELS:"Aselsan",
KCHOL:"Koç H.",ISCTR:"İş Bankası",VAKBN:"Vakıfbank",HALKB:"Halk Bank",YKBNK:"Yapı Kredi",
FROTO:"Ford Otosan",TOASO:"Tofaş",BIMAS:"BİM",MGROS:"Migros",ARCLK:"Arçelik",
TUPRS:"Tüpraş",PETKM:"Petkim",SASA:"Sasa",EKGYO:"Emlak GYO",TTKOM:"T.Telekom",
TCELL:"Turkcell",PGSUS:"Pegasus",TAVHL:"TAV",ENKAI:"Enka",KOZAL:"Koza Altın",
KOZAA:"Koza Anadolu",AEFES:"Anadolu Efes",CCOLA:"Coca-Cola",ULKER:"Ülker",LOGO:"Logo",
MAVI:"Mavi",OTKAR:"Otokar",TTRAK:"T.Traktör",GUBRF:"Gübre Fab.",AKCNS:"Akçansa",
CIMSA:"Çimsa",NUHCM:"Nuh Çimento",BRISA:"Brisa",KORDS:"Kordsa",AYGAZ:"Aygaz",
DOHOL:"Doğan H.",DOAS:"Doğuş Oto",SKBNK:"Şekerbank",ALBRK:"Albaraka",QNBFB:"QNB Finans",
TSKB:"TSKB",ENJSA:"Enerjisa",AKSEN:"Aksa Enerji",AKENR:"Ak Enerji",GWIND:"Galata Wind",
ODAS:"Odaş",ZOREN:"Zorlu Enerji",ASTOR:"Astor Enerji",MPARK:"MLP Sağlık",TKFEN:"Tekfen",
ALKIM:"Alkim",SISE:"Şişecam",ANACM:"Anadolu Cam"};

// dedupe SL
SL=SL.filter(function(s,i){return SL.indexOf(s)===i;});

var CUR="THYAO",TF="1d",TFR="1y",LTAB="fiyat";
var PRICE={},CHG={},SIG={},PF=[],PFS="",SD=true,BARS=null,MDIR={};
...
// Sinyal hesapla (Yahoo Finance veriden)
function calcSig(chgPct, closeVsSma) {
  // chgPct: günlük değişim %, closeVsSma: fiyat SMA üzerinde mi (1/-1/0)
  var score = 0;
  if (chgPct > 3) score += 2;
  else if (chgPct > 1) score += 1;
  else if (chgPct < -3) score -= 2;
  else if (chgPct < -1) score -= 1;
  if (closeVsSma > 0) score += 1;
  else if (closeVsSma < 0) score -= 1;
  if (score >= 3) return 2;   // G.AL
  if (score >= 1) return 1;   // AL
  if (score <= -3) return -2; // G.SAT
  if (score <= -1) return -1; // SAT
  return 0;                   // NÖTR
}

function sigLabel(v) {
  if (v === 2) return {t:"G.AL", bg:"#14532d", c:"#22c55e"};
  if (v === 1) return {t:"AL",   bg:"#052e16", c:"#86efac"};
  if (v === 0) return {t:"NÖTR", bg:"#1e293b", c:"#94a3b8"};
  if (v ===-1) return {t:"SAT",  bg:"#3b1010", c:"#fca5a5"};
  return              {t:"G.SAT",bg:"#450a0a", c:"#ef4444"};
}

function showP(id){
  ["tp-sinyal","tp-fiyat","tp-portfoy"].forEach(function(p){
    var e=G(p);if(e){e.classList.remove("pnl");e.classList.add("hid");}
  });
...
function buildWL(q){
  ...
  var f=SL.filter(function(s){return!q||s.includes(q)||(NAMES[s]||"").toUpperCase().includes(q);});
  var wlc=G("wlc");if(wlc)wlc.textContent=f.length+" hisse";
  var h="";
  f.forEach(function(s){
    var sig=sigLabel(SIG[s]);var chg=CHG[s];
    ...
    h+='<div onclick="pick(\''+s+'\')" style="padding:8px 9px;border-bottom:1px solid #0d1018;cursor:pointer;background:'+(ic?"#0f1a2a":"transparent")+';display:flex;align-items:center;gap:4px;">'
     ...
     +'<div style="font-size:11px;color:#7a9ab8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'+(NAMES[s]||s)+'</div></div>'
     +'<div style="text-align:right;flex-shrink:0;">'
     +'<div style="font-size:9px;font-weight:700;background:'+sig.bg+';color:'+sig.c+';padding:1px 4px;">'+sig.t+'</div>'
...
function goTab(id,btn){
  ...
  if(btn)btn.classList.add("on");showP("tp-"+id);LTAB=id;
...
// ═══════════════════════════════════════════════════════════════════
// VERİ ÇEKME — SUNUCU PROXYLERİ ÜZERINDEN
// ═══════════════════════════════════════════════════════════════════

// Yahoo Finance çoklu fiyat — Railway sunucu üzerinden
async function fetchQuotes(symList) {
  const chunks = [];
  for (let i = 0; i < symList.length; i += 20) chunks.push(symList.slice(i, i + 20));
  for (const chunk of chunks) {
    try {
      const r = await fetch('/api/quotes?symbols=' + chunk.join(','));
      ...
      if (quotes && quotes.length > 0) {
        quotes.forEach(function(q) {
          ...
          if (sym) {
            PRICE[sym] = q.regularMarketPrice || 0;
            CHG[sym] = q.regularMarketChangePercent || 0;
            SIG[sym] = calcSig(CHG[sym], 0);
          }
        });
      }
    } catch(e) { console.log('quotes chunk err:', e.message); }
  }
}

// TV Scanner — Railway sunucu /api/scanner
async function fetchScanner(body) {
...
async function refreshAll() {
  setSt(null, "Veriler yükleniyor...");
  buildWL(G("wls") ? G("wls").value : "");
  renderF();

  // 1. TV Scanner dene (sunucu proxy)
  let scannerOK = false;
  try {
    const data = await fetchScanner({
      filter: [], options: { lang: "tr" },
      columns: ["name","Recommend.All","close","change"],
      sort: { sortBy: "change", sortOrder: "desc" },
      range: [0, 700], markets: ["turkey"]
    });
    if (data && data.data && data.data.length > 0) {
      // TV scanner'dan yeni hisseler ekle
      const existing = new Set(SL);
      data.data.forEach(function(item) {
        const sym = (item.s || "").replace(/^BIST(|_DL):/, "").trim();
        const d = item.d;
        if (!sym || !d) return;
        if (!existing.has(sym)) { SL.push(sym); existing.add(sym); }
        if (d[3] != null) CHG[sym] = parseFloat(d[3].toFixed(2));
        // Recommend.All'dan sinyal
        const rec = d[1] != null ? parseFloat(d[1]) : null;
        if (rec != null) {
          if (rec >= 0.5) SIG[sym] = 2;
          else if (rec >= 0.1) SIG[sym] = 1;
          else if (rec <= -0.5) SIG[sym] = -2;
          else if (rec <= -0.1) SIG[sym] = -1;
          else SIG[sym] = 0;
        }
      });
      scannerOK = true;
      setSt(true, "✅ TV Scanner — " + SL.length + " hisse · " + new Date().toLocaleTimeString("tr-TR"));
      const sl2 = G("src-lbl"); if(sl2) sl2.textContent = "· TV Scanner";
    }
  } catch (e) {
    console.log("TV Scanner başarısız:", e.message);
  }

  // 2. TV Scanner başarısız olduysa Yahoo Finance multi-quote kullan
  if (!scannerOK) {
    console.log("Yahoo Finance multi-quote fallback başlatılıyor...");
    try {
      await fetchQuotes(SL);
      setSt(true, "✅ Yahoo Finance — " + SL.length + " hisse · " + new Date().toLocaleTimeString("tr-TR"));
      const sl2 = G("src-lbl"); if(sl2) sl2.textContent = "· Yahoo Finance";
    } catch (e) {
      setSt(false, "❌ Veri alınamadı: " + e.message);
    }
  }
  ...
  // Aylık scanner arka planda
  setTimeout(fetchMonthly, 2000);
}
...
async function fetchMonthly() {
  try {
    const data = await fetchScanner({
      filter:[], options:{lang:"tr"},
      columns:["name","Recommend.All|1M","RSI|1M","EMA20|1M","EMA50|1M","MACD.hist|1M"],
      sort:{sortBy:"name",sortOrder:"asc"}, range:[0,700], markets:["turkey"]
    });
    if (!data || !data.data) return;
    data.data.forEach(function(item) {
      const sym = (item.s||"").replace(/^BIST(|_DL):/,"").trim();
      const d = item.d; if(!sym||!d) return;
      const rec=parseFloat(d[1]),r2=parseFloat(d[2]),em20=parseFloat(d[3]),em50=parseFloat(d[4]),hist=parseFloat(d[5]);
      if(!isNaN(rec)) {
        const f1=rec>=0.2,f2=!isNaN(r2)&&r2>=50,f3=!isNaN(em20)&&!isNaN(em50)&&em20>em50,f4=!isNaN(hist)&&hist>0;
        const sc=(f1?1:0)+(f2?1:0)+(f3?1:0)+(f4?1:0);
        // monthly direction for chart
        MDIR[sym] = (sc>=3) ? 1 : -1;
      }
    });
  } catch(e) { console.log("monthly err:", e.message); }
}
...
function drawChart(bars,mdir){
  ...
  var C=bars.map(function(d){return d.c;}),HH=bars.map(function(d){return d.h;}),
      LL=bars.map(function(d){return d.l;}),OO=bars.map(function(d){return d.o;}),
      VV=bars.map(function(d){return d.v;});
  var rA=calcRSI(C,14),mR=calcMACD(C),bR=calcBB(C,20,2),
      sA=calcStoch(HH,LL,C,14),aA=calcATR(HH,LL,C,14),sD=calcST(HH,LL,C,10,3);
  ...
  sc("irsi",lRSI.toFixed(1),lRSI<35?"bull":lRSI>70?"bear":"neu");sc("istoch",lSt.toFixed(1),lSt<20?"bull":lSt>80?"bear":"neu");sc("imacd",(lM>0?"▲":"▼")+Math.abs(lM).toFixed(3),lM>0&&lM>lMp?"bull":lM<0?"bear":"neu");sc("ibb",price<lBL?"ALT":price>lBU?"ÜST":bW<bWp*0.9?"SIKIŞ":"ORTA",price<lBL?"bull":price>lBU?"bear":"neu");sc("iatr",(lA/price*100).toFixed(1)+"%","neu");sc("ivwap",price>lVW?"Üst":"Alt",price>lVW?"bull":"bear");sc("ist",lSD===1?"YEŞİL":"KIRMIZI",lSD===1?"bull":"bear");
  ...
  var rrr=((t2-price)/(price-stop2)).toFixed(1);
  function se(id,v,c){var e=G(id);if(e){e.textContent=v;if(c)e.style.color=c;}}
  se("vee","₺"+price.toFixed(2),"#00d4ff");se("ves","₺"+stop2.toFixed(2),"#ef4444");se("veh1","₺"+t1.toFixed(2),"#e8b84b");se("veh2","₺"+t2.toFixed(2),"#22c55e");se("veh3","₺"+t3.toFixed(2),"#22c55e");se("verr","1:"+rrr,rrr>=2?"#22c55e":"#ef4444");
  // ── DRAW ──
  ...
  for(var i=n-1;i>=0;i--){if(bR.l[i]==null)continue;mc.lineTo(px(i),py(bR.l[i]));}mc.closePath();mc.fill();
...
// ═══════════════════════════════════════════════════════════════════
// CHART YÜKLEMESİ — Railway /api/:sym üzerinden
// ═══════════════════════════════════════════════════════════════════
function loadChart(){
  var sym=CUR,msg=G("cmsg"),ct=G("cmt"),cs=G("cms");
  if(msg)msg.style.display="flex";
  if(ct)ct.textContent="⏳ "+sym+" grafik yükleniyor...";
  if(cs)cs.textContent="Yahoo Finance proxy · Railway";
...
function renderF(){
  ...
  rows.forEach(function(r){
    ...
    parts.push('<div onclick="pick(\''+r.sym+'\')" style="display:grid;grid-template-columns:66px 1fr 70px;padding:6px 8px;border-bottom:1px solid #0d1018;cursor:pointer;background:'+(ic?"#0f1a2a":"transparent")+'">'
      ...
      +'<span style="text-align:center"><span style="font-size:10px;font-weight:700;padding:1px 6px;background:'+sl2.bg+';color:'+sl2.c+';">'+sl2.t+'</span></span>'
...
// PORTFÖY
// ═══════════════════════════════════════════════════════════════════
function spf(q){var box=G("pfsug");if(!box)return;if(!q){box.style.display="none";return;}q=q.toUpperCase();var m=SL.filter(function(s){return s.includes(q)||(NAMES[s]||"").toUpperCase().includes(q);}).slice(0,8);if(!m.length){box.style.display="none";return;}box.style.display="block";box.innerHTML=m.map(function(s){return'<div style="padding:8px 10px;cursor:pointer;font-size:12px;border-bottom:1px solid #0f1520;color:#fff;" onclick="selPF(\''+s+'\')">'+s+' <span style="color:#4b5e78;font-size:10px;">'+(NAMES[s]||"")+'</span></div>';}).join("");}
function selPF(sym){PFS=sym;var lbl=G("pflbl");if(lbl)lbl.textContent="✅ "+sym+" — "+(NAMES[sym]||"");var sug=G("pfsug");if(sug)sug.style.display="none";var inp=G("pfsi");if(inp)inp.value=sym;}
function pfAdd(){if(!PFS){alert("Önce hisse seç!");return;}if(PF.find(function(p){return p.sym===PFS;})){alert("Zaten portföyde!");return;}PF.push({id:Date.now(),sym:PFS});savePF();renderPF();}
function pfRem(){if(!PFS){alert("Önce hisse seç!");return;}PF=PF.filter(function(p){return p.sym!==PFS;});PFS="";savePF();renderPF();}
function pfDel(id){PF=PF.filter(function(p){return p.id!==id;});savePF();renderPF();}
function savePF(){try{localStorage.setItem("bist_pf7",JSON.stringify(PF));}catch(e){}}
function loadPF(){try{var j=localStorage.getItem("bist_pf7");if(j)PF=JSON.parse(j);}catch(e){}}
function renderPF(){
  ...
  PF.forEach(function(p){
    var sl2=sigLabel(SIG[p.sym]||0);var chg=CHG[p.sym];
    ...
    html+='<div style="padding:10px 12px;border-bottom:1px solid #0f1520;display:flex;align-items:center;justify-content:space-between;">'
     ...
     +'<span style="font-size:9px;font-weight:700;padding:1px 6px;background:'+sl2.bg+';color:'+sl2.c+';">'+sl2.t+'</span>'
     +'<span style="font-size:11px;font-weight:700;color:'+cc+';">'+cs+'</span>'
     +'</div><div style="font-size:10px;color:#3d5878;">'+(NAMES[p.sym]||p.sym)+'</div></div>'

...
// ─── TV SCANNER PROXY (sunucu tarafı, CORS sorunu yok) ───────────────────────
app.post('/api/scanner', (req, res) => {
  ...
  const r = https.request(options, (yr) => {
    let d = '';
    yr.on('data', c => d += c);
    yr.on('end', () => {
...
// ─── YAHOO FINANCE PROXY ─────────────────────────────────────────────────────
function yahooFetch(sym, interval, range, res) {
  ...
  const fallback = () => {
    ...
    https.get(url2, opts, yr => {
      let d = ''; yr.on('data', c => d += c);
      yr.on('end', () => { res.setHeader('Content-Type','application/json'); res.end(d); });
    }).on('error', e => res.status(500).json({ error: e.message }));
  };
  https.get(url, opts, yr => {
    let d = ''; yr.on('data', c => d += c);
    yr.on('end', () => {
      if (yr.statusCode === 200) { res.setHeader('Content-Type','application/json'); res.end(d); }
      else fallback();
    });
  }).on('error', () => fallback());
}

app.get('/api/monthly/:sym', (req, res) => yahooFetch(req.params.sym.toUpperCase(), '1mo', '3y', res));
app.get('/api/:sym', (req, res) => yahooFetch(req.params.sym.toUpperCase(), req.query.tf || '1d', req.query.range || '1y', res));

// ─── ANA SAYFA ───────────────────────────────────────────────────────────────
app.get('/', (req, res) => { res.setHeader('Content-Type','text/html; charset=utf-8'); res.send(HTML); });

app.listen(PORT, () => console.log('VORTEX OK port:' + PORT));
...
.sep{width:1px;height:18px;background:#1a2030}
...
#wl input{width:100%;background:#0f1118;border:none;border-bottom:1px solid #131826;color:#fff;padding:8px 9px;font-size:11px;font-family:inherit;outline:none}
...
#cmsg{position:absolute;inset:0;background:rgba(7,9,15,.95);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;z-index:10;font-size:11px;color:#4b5e78}
...
/* fiyat */
#ftable{flex:1;overflow-y:auto}
/* portfoy */
#pfadd{padding:8px 10px;border-bottom:1px solid #131826;flex-shrink:0}
#pfadd input{width:100%;background:#060a10;border:1px solid #1e3050;color:#fff;padding:7px 9px;font-size:12px;font-family:inherit;outline:none;border-radius:4px;margin-bottom:6px}
.pfsug div{padding:8px 10px;cursor:pointer;font-size:12px;border-bottom:1px solid #0f1520;color:#fff}
</style>
</head><body>
...
<div id="body">
  ...
  <div id="center">
    ...
    <div id="tp-portfoy" class="pnl hid">
      ...
      <div id="pfadd">
        <input id="pfsi" placeholder="🔍 Hisse ara..." oninput="spf(this.value)">
        <div id="pfsug" class="pfsug" style="background:#0b1424;border:1px solid #1e3050;border-top:none;display:none;max-height:130px;overflow-y:auto;border-radius:0 0 4px 4px;margin-bottom:6px;"></div>
...
{s:"CCOLA",n:"Coca-Cola"},{s:"ULKER",n:"Ülker"},{s:"LOGO",n:"Logo"},{s:"MAVI",n:"Mavi"},
{s:"OTKAR",n:"Otokar"},{s:"TTRAK",n:"T.Traktör"},{s:"GUBRF",n:"Gübre Fab."},
{s:"AKCNS",n:"Akçansa"},{s:"CIMSA",n:"Çimsa"},{s:"NUHCM",n:"Nuh Çimento"},
{s:"BRISA",n:"Brisa"},{s:"KORDS",n:"Kordsa"},{s:"AYGAZ",n:"Aygaz"},
{s:"DOHOL",n:"Doğan H."},{s:"DOAS",n:"Doğuş Oto"},{s:"SKBNK",n:"Şekerbank"},
{s:"ALBRK",n:"Albaraka"},{s:"QNBFB",n:"QNB Finans"},{s:"TSKB",n:"TSKB"},
{s:"ENJSA",n:"Enerjisa"},{s:"AKSEN",n:"Aksa Enerji"},{s:"AKENR",n:"Ak Enerji"},
{s:"GWIND",n:"Galata Wind"},{s:"ODAS",n:"Odaş"},{s:"ZOREN",n:"Zorlu Enerji"},
{s:"ASTOR",n:"Astor Enerji"},{s:"MPARK",n:"MLP Sağlık"},{s:"TKFEN",n:"Tekfen"},
{s:"ALKIM",n:"Alkim"},{s:"SISE",n:"Şişecam"},{s:"ANACM",n:"Anadolu Cam"},
{s:"A1CAP",n:"A1 Capital"},{s:"AKGRT",n:"Aksigorta"},{s:"AKSA",n:"Aksa Akrilik"},
{s:"ALARK",n:"Alarko"},{s:"ANSGR",n:"Anadolu Sig."},{s:"ARENA",n:"Arena BT"},
{s:"ASUZU",n:"Anadolu Isuzu"},{s:"BANVT",n:"Banvit"},{s:"BERA",n:"Bera H."},
{s:"BFREN",n:"Bosch Fren"},{s:"BIZIM",n:"Bizim Top."},{s:"BJKAS",n:"Beşiktaş"},
{s:"BRSAN",n:"Borusan Çelik"},{s:"BTCIM",n:"Batıçimento"},{s:"BUCIM",n:"Bursa Çim."},
{s:"CRFSA",n:"CarrefourSA"},{s:"DARDL",n:"Dardanel"},{s:"DEVA",n:"Deva H."},
{s:"ECILC",n:"Eczacıbaşı İl."},{s:"ERCB",n:"Erçiyas Çim."},{s:"EUPWR",n:"Europower"},
{s:"FENER",n:"Fenerbahçe"},{s:"GARFA",n:"Garanti Fakt."},{s:"GESAN",n:"Gersan El."},
{s:"GSDHO",n:"GSD Holding"},{s:"GSRAY",n:"Galatasaray"},{s:"HEKTS",n:"Hektaş"},
{s:"HLGYO",n:"Halk GYO"},{s:"HOROZ",n:"Horoz Loj."},{s:"IHLAS",n:"İhlas H."},
{s:"INDES",n:"İndeks BT"},{s:"ISATR",n:"İş Portföy"},{s:"ISGYO",n:"İş GYO"},
{s:"IZMDC",n:"İzmit Demir"},{s:"KARSN",n:"Karsan"},{s:"KARTN",n:"Kartonsan"},
{s:"KONTR",n:"Kontrolmatik"},{s:"KONYA",n:"Konya Çim."},{s:"KRDMD",n:"Kardemir D"},
{s:"MUTLU",n:"Mutlu Akü"},{s:"OYAKC",n:"Oyak Çim."},{s:"PAPIL",n:"Papilon"},
{s:"POLHO",n:"Polisan H."},{s:"PRKAB",n:"T.Prysmian"},{s:"RAYSG",n:"Ray Sigorta"},
{s:"RNSHO",n:"Rönesans H."},{s:"RYSAS",n:"Reysaş"},{s:"SELEC",n:"Selçuk Ecza"},
{s:"SNGYO",n:"Sinpaş GYO"},{s:"TRGYO",n:"Torunlar GYO"},{s:"TURSG",n:"T.Sigorta"},
{s:"VESTL",n:"Vestel"},{s:"VKGYO",n:"Vakıf GYO"},{s:"YYLGD",n:"Yayla Agro"},
{s:"BAGFS",n:"Bagfaş"},{s:"AGHOL",n:"AG Anadolu"},{s:"AHGAZ",n:"Ahlatcı Gaz"}
];
// dedupe
var _m={};SL=SL.filter(function(s){if(_m[s.s])return false;_m[s.s]=1;return true;});

var CUR="THYAO",TF="1d",TFR="1y",LTAB="fiyat";
var REC={},CHG={},STM={},PF=[],PFS="",API=[],SD=true,BARS=null,MDIR={};

setInterval(function(){var e=$("clk");if(e)e.textContent=new Date().toLocaleTimeString("tr-TR");},1000);
function $(id){return document.getElementById(id);}
function setSt(ok,msg){var d=$("dot"),m=$("smsg");if(d)d.style.background=ok===true?"#22c55e":ok===false?"#ef4444":"#e8b84b";if(m){m.textContent=msg;m.style.color=ok===true?"#22c55e":ok===false?"#ef4444":"#e8b84b";}}
function rl(v){if(v==null)return{t:"—",bg:"#1e293b",c:"#475569"};if(v<=-0.5)return{t:"G.SAT",bg:"#450a0a",c:"#ef4444"};if(v<-0.1)return{t:"SAT",bg:"#3b1010",c:"#fca5a5"};if(v<=0.1)return{t:"NÖTR",bg:"#1e293b",c:"#94a3b8"};if(v<0.5)return{t:"AL",bg:"#052e16",c:"#86efac"};return{t:"G.AL",bg:"#14532d",c:"#22c55e"};}
function showP(id){["tp-sinyal","tp-fiyat","tp-portfoy"].forEach(function(p){var e=$(p);if(e){e.classList.remove("pnl");e.classList.add("hid");}});var e=$(id);if(e){e.classList.remove("hid");e.classList.add("pnl");}}
function buildWL(q){
  q=(q||"").toUpperCase().trim();
  var f=SL.filter(function(s){return!q||s.s.includes(q)||s.n.toUpperCase().includes(q);});
  var wlc=$("wlc");if(wlc)wlc.textContent=f.length+" hisse";
  var h="";
  f.forEach(function(s){
    var r2=rl(REC[s.s]);var chg=CHG[s.s];
    ...
    h+='<div onclick="pick(\''+s.s+'\')" style="padding:8px 9px;border-bottom:1px solid #0d1018;cursor:pointer;background:'+(ic?"#0f1a2a":"transparent")+';display:flex;align-items:center;gap:4px;">'
     +'<div style="flex:1;min-width:0;">'
       +'<div style="font-size:14px;font-weight:700;color:'+(ic?"#e8b84b":"#fff")+';">'+s.s+'</div>'
       +'<div style="font-size:11px;color:#7a9ab8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'+s.n+'</div>'
     +'</div>'
     +'<div style="text-align:right;flex-shrink:0;">'
       +'<div style="font-size:9px;font-weight:700;background:'+r2.bg+';color:'+r2.c+';padding:1px 4px;">'+r2.t+'</div>'
       +(cs?'<div style="font-size:9px;color:'+cc+';">'+cs+'</div>':"")
     +'</div></div>';
  });
  var wll=$("wll");if(wll)wll.innerHTML=h;
}
...
function pick(s){
  CUR=s;var h=$("hsym");if(h)h.textContent=s;
  buildWL($("wls").value||"");
  showP("tp-sinyal");
  document.querySelectorAll(".tab").forEach(function(b){b.classList.remove("on");});
...
function setTF(tf,r,btn){TF=tf;TFR=r;document.querySelectorAll(".tfb").forEach(function(b){b.classList.remove("on");});if(btn)btn.classList.add("on");loadChart();}
...
function rsi(c,p){p=p||14;var r=[],g=0,l=0;for(var i=1;i<c.length;i++){var d=c[i]-c[i-1];if(i<=p){g+=Math.max(d,0);l+=Math.max(-d,0);if(i===p)r.push(l===0?100:100-100/(1+g/l));else r.push(null);}else{g=(g*(p-1)+Math.max(d,0))/p;l=(l*(p-1)+Math.max(-d,0))/p;r.push(l===0?100:100-100/(1+g/l));}}return r;}
function macd(c){var e12=ema(c,12),e26=ema(c,26);var ml=c.map(function(_,i){return e12[i]!=null&&e26[i]!=null?e12[i]-e26[i]:null;});var sig=ema(ml.filter(function(v){return v!=null;}),9);var h=[],si=0;for(var i=0;i<ml.length;i++){if(ml[i]==null)h.push(null);else{h.push(sig[si]!=null?ml[i]-sig[si]:null);si++;}}return{ml:ml,h:h};}
function bb(c,p,m){p=p||20;m=m||2;var u=[],l=[],md=[];for(var i=0;i<c.length;i++){if(i<p-1){u.push(null);l.push(null);md.push(null);continue;}var s=c.slice(i-p+1,i+1);var mn=s.reduce(function(a,b){return a+b;})/p;var sd=Math.sqrt(s.reduce(function(a,b){return a+(b-mn)*(b-mn);},0)/p);u.push(mn+m*sd);l.push(mn-m*sd);md.push(mn);}return{u:u,l:l,m:md};}
function stoch(H,L,C,k){k=k||14;var r=[];for(var i=0;i<C.length;i++){if(i<k-1){r.push(null);continue;}var hh=Math.max.apply(null,H.slice(i-k+1,i+1));var ll=Math.min.apply(null,L.slice(i-k+1,i+1));r.push(hh===ll?50:((C[i]-ll)/(hh-ll))*100);}return r;}
function atr(H,L,C,p){p=p||14;var tr=[],at=[];for(var i=0;i<C.length;i++){if(i===0){tr.push(H[i]-L[i]);at.push(null);continue;}tr.push(Math.max(H[i]-L[i],Math.abs(H[i]-C[i-1]),Math.abs(L[i]-C[i-1])));if(i<p)at.push(null);else if(i===p)at.push(tr.slice(0,p+1).reduce(function(a,b){return a+b;})/p);else at.push((at[i-1]*(p-1)+tr[i])/p);}return at;}
function st(H,L,C,p,m){p=p||10;m=m||3;var a=atr(H,L,C,p),dr=[],ps=null,pd=1;for(var i=0;i<C.length;i++){if(a[i]==null){dr.push(null);continue;}var hl=(H[i]+L[i])/2,up=hl+m*a[i],dn=hl-m*a[i];var cs,cd;if(ps==null){cs=up;cd=1;}else if(pd===1){cs=C[i]>ps?Math.max(dn,ps):up;cd=C[i]>cs?1:-1;}else{cs=C[i]<ps?Math.min(up,ps):dn;cd=C[i]>cs?1:-1;}dr.push(cd);ps=cs;pd=cd;}return dr;}
function obv(C,V){var o=[V[0]||0];for(var i=1;i<C.length;i++)o.push(C[i]>C[i-1]?o[i-1]+(V[i]||0):C[i]<C[i-1]?o[i-1]-(V[i]||0):o[i-1]);return o;}
...
function drawChart(bars,mdir){
  var co=$("couter"),cm=$("cvM"),cr=$("cvR");if(!co||!cm||!cr)return;
  var TW=co.clientWidth||320,TH=co.clientHeight-60||200;if(TH<80)TH=80;
  ...
  var mc=cm.getContext("2d");mc.scale(dpr,dpr);var rc=cr.getContext("2d");rc.scale(dpr,dpr);
  ...
  var rA=rsi(C,14),mR=macd(C),bR=bb(C,20,2),sA=stoch(HH,LL,C,14),aA=atr(HH,LL,C,14),sD=st(HH,LL,C,10,3);
  var de25=dema(C,25),e8=ema(C,8),e21=ema(C,21),e50=ema(C,50),oA=obv(C,VV);
  ...
  var lS=lv(sA)||50,lA=lv(aA)||0,lST=lv(sD)||1;
  var lOBV=oA[n-1],pOBV=oA[Math.max(0,n-6)];
  var lDE=lv(de25)||0,lE8=lv(e8)||0,lE21=lv(e21)||0,lE50=lv(e50)||0,lVW=vA[n-1],price=C[n-1];
  ...
  var I={rsi:lRSI,stoch:lS,macd:lM,macdP:lMp,div:div,mst:mdir||1,dema:lDE,e8:lE8,e21:lE21,e50:lE50,stdir:lST,price:price,bbu:lBU,bbl:lBL,bbm:lBM,squeeze:bW<bWp*0.9,z:z,volr:volR,vwap:lVW,obvUp:lOBV>pOBV,fib:fib,atr:lA};
  var SC=vScore(I);
  var bc=SC.f>=75?"#22c55e":SC.f>=60?"#00d4ff":SC.f>=40?"#e8b84b":"#ef4444";
  var bt=SC.f>=75?"GÜÇLÜ AL ▲":SC.f>=60?"İZLE ◆":SC.f>=40?"BEKLE ■":"UZAK DUR ▼";
  var bg=$("vfbadge");if(bg){bg.style.color=bc;bg.style.borderColor=bc;bg.textContent="VF:"+SC.f+" "+bt;}
  function sv(id,v,cls){var e=$(id);if(e){e.textContent=v;e.className="vfv "+(cls||"");}}
  sv("vmom",SC.mom,SC.mom>=60?"bull":SC.mom<40?"bear":"neu");sv("vtr",SC.tr,SC.tr>=60?"bull":SC.tr<40?"bear":"neu");sv("vvol",SC.vol,SC.vol>=60?"bull":SC.vol<40?"bear":"neu");sv("vhac",SC.hac,SC.hac>=60?"bull":SC.hac<40?"bear":"neu");
  function sc(id,v,cls){var e=$(id);if(e){e.textContent=v;e.parentElement.className="ic "+(cls||"dim");}}
  sc("irsi",lRSI.toFixed(1),lRSI<35?"bull":lRSI>70?"bear":"neu");sc("istoch",lS.toFixed(1),lS<20?"bull":lS>80?"bear":"neu");sc("imacd",(lM>0?"▲":"▼")+Math.abs(lM).toFixed(3),lM>0&&lM>lMp?"bull":lM<0?"bear":"neu");sc("ibb",price<lBL?"ALT":price>lBU?"ÜST":bW<bWp*0.9?"SIKIŞ":"ORTA",price<lBL?"bull":price>lBU?"bear":"neu");sc("iatr",(lA/price*100).toFixed(1)+"%","neu");sc("ivwap",price>lVW?"Üst":"Alt",price>lVW?"bull":"bear");sc("ist",lST===1?"YEŞİL":"KIRMIZI",lST===1?"bull":"bear");
  var prev=bars[n-2]?bars[n-2].c:price,chg=price-prev,pl=$("vplbl");
  ...
  var rr2b=((t2-price)/(price-stop2)).toFixed(1);
  function se(id,v,c){var e=$(id);if(e){e.textContent=v;if(c)e.style.color=c;}}
  se("vee","₺"+price.toFixed(2),"#00d4ff");se("ves","₺"+stop2.toFixed(2),"#ef4444");se("veh1","₺"+t1.toFixed(2),"#e8b84b");se("veh2","₺"+t2.toFixed(2),"#22c55e");se("veh3","₺"+t3.toFixed(2),"#22c55e");se("verr","1:"+rr2b,rr2b>=2?"#22c55e":"#ef4444");
  // draw
  ...
  var pMi=Math.min.apply(null,allP.filter(function(v){return v>0;})),pMa=Math.max.apply(null,allP),pR=pMa-pMi||1;pMi-=pR*0.03;pMa+=pR*0.18;
  var VH=20,pd={l:2,r:52,t:6,b:18};
  function px(i){return pd.l+(i/(n-1||1))*(W-pd.l-pd.r);}function py(v){return pd.t+(1-(v-pMi)/(pMa-pMi))*(H-pd.t-pd.b-VH);}
  mc.fillStyle="#07090f";mc.fillRect(0,0,W,H);
  mc.strokeStyle="#0d1018";mc.lineWidth=0.5;
  for(var gi=0;gi<5;gi++){var gv=pMi+(pMa-pMi)*gi/4;var gy=py(gv);mc.beginPath();mc.moveTo(pd.l,gy);mc.lineTo(W-pd.r,gy);mc.stroke();mc.fillStyle="#2a3548";mc.font="9px monospace";mc.textAlign="right";mc.fillText(gv.toFixed(gv>100?0:gv>10?1:2),W-2,gy+3);}
  // ST tint
  var ss2=0,sd2=null;for(var i=0;i<=n;i++){var dd=sD[i]||null;if(dd!==sd2||i===n){if(sd2!=null&&i>ss2){mc.fillStyle=sd2===1?"rgba(34,197,94,0.05)":"rgba(239,68,68,0.05)";mc.fillRect(px(ss2),pd.t,px(Math.min(i,n-1))-px(ss2),H-pd.t-pd.b-VH);}sd2=dd;ss2=i;}}
  // BB fill
  mc.fillStyle="rgba(0,212,255,0.04)";mc.beginPath();var fs=true;
  ...
  dl(vA,"rgba(180,127,255,.8)",1,[2,2]);dl(e50,"rgba(180,127,255,.5)",1,[4,2]);dl(de25,"#e8b84b",.9,[]);
  ...
  var cw2=Math.max(1,(W-pd.l-pd.r)/n*.72);
  for(var i=0;i<n;i++){var x=px(i);var up=C[i]>=OO[i];var col=up?"#22c55e":"#ef4444";mc.strokeStyle=col;mc.lineWidth=1;mc.beginPath();mc.moveTo(x,py(HH[i]));mc.lineTo(x,py(LL[i]));mc.stroke();mc.fillStyle=col;var bt2=py(Math.max(C[i],OO[i])),bb2=py(Math.min(C[i],OO[i]));mc.fillRect(x-cw2/2,bt2,cw2,Math.max(1,bb2-bt2));}
  ...
  for(var i=0;i<n;i++){var x=px(i),vh=(VV[i]/(mxV||1))*VH*.9;mc.fillStyle=C[i]>=OO[i]?"rgba(34,197,94,.4)":"rgba(239,68,68,.4)";mc.fillRect(x-cw2/2,H-pd.b-vh,cw2,vh);}
  ...
  [30,50,70].forEach(function(lv2){var y2=ry(lv2);rc.beginPath();rc.moveTo(0,y2);rc.lineTo(W-40,y2);rc.stroke();rc.fillStyle=lv2===30?"#22c55e":lv2===70?"#ef4444":"#2a3548";rc.font="8px monospace";rc.textAlign="left";rc.fillText(lv2,W-38,y2+3);});
...
// ─── LOAD CHART ──────────────────────────────────────────────────────────────
function loadChart(){
  var sym=CUR,msg=$("cmsg"),ct=$("cmt"),cs=$("cms");
  ...
  fetch("/api/monthly/"+sym).then(function(r){return r.json();}).then(function(data){
    var ch=data&&data.chart;if(ch&&ch.result&&ch.result[0]){var res=ch.result[0],q=res.indicators.quote[0];var mv=[];for(var i=0;i<res.timestamp.length;i++)if(q.close[i]!=null&&q.high[i]!=null&&q.low[i]!=null)mv.push({h:q.high[i],l:q.low[i],c:q.close[i]});if(mv.length>=10){var mST=st(mv.map(function(d){return d.h;}),mv.map(function(d){return d.l;}),mv.map(function(d){return d.c;}),10,3);MDIR[sym]=lv(mST)||1;}}
  }).catch(function(){}).finally(function(){
    fetch("/api/"+sym+"?tf="+TF+"&range="+TFR).then(function(r){return r.json();}).then(function(data){
      var ch=data&&data.chart;if(!ch||!ch.result||!ch.result[0]){if(msg){msg.style.display="flex";if(ct)ct.textContent="⚠ Veri alınamadı";}return;}
      var res=ch.result[0],q=res.indicators.quote[0];
      var v=[];for(var i=0;i<res.timestamp.length;i++)if(q.close[i]!=null&&q.high[i]!=null&&q.low[i]!=null&&q.open[i]!=null)v.push({t:res.timestamp[i],o:q.open[i],h:q.high[i],l:q.low[i],c:q.close[i],v:q.volume[i]||0});
      if(v.length<5){if(msg){msg.style.display="flex";if(ct)ct.textContent="⚠ Yetersiz veri";}return;}
      if(msg)msg.style.display="none";BARS=v;drawChart(v,MDIR[sym]||1);
...
// ─── TV SCANNER — sunucu proxy üzerinden ─────────────────────────────────────
async function scanner(body){
  const r=await fetch('/api/scanner',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
...
async function fetchRecs(){
  ...
  try{
    var data=await scanner({filter:[],options:{lang:"tr"},columns:["name","Recommend.All","close","change"],sort:{sortBy:"change",sortOrder:"desc"},range:[0,700],markets:["turkey"]});
    API=[];
    data.data.forEach(function(item){
      var sym=(item.s||"").replace(/^BIST(|_DL):/,"").trim();var d=item.d;if(!sym||!d)return;
      if(d[1]!=null)REC[sym]=parseFloat(d[1]);if(d[3]!=null)CHG[sym]=parseFloat(d[3].toFixed(2));
    ...
    buildWL($("wls")?$("wls").value:"");SD=true;renderF();renderPF();
  }catch(e){setSt(false,"❌ "+e.message);}
}
async function fetchMonthly(){
  try{
    var data=await scanner({filter:[],options:{lang:"tr"},columns:["name","Recommend.All|1M","RSI|1M","EMA20|1M","EMA50|1M","MACD.hist|1M"],sort:{sortBy:"name",sortOrder:"asc"},range:[0,700],markets:["turkey"]});
    data.data.forEach(function(item){
      var sym=(item.s||"").replace(/^BIST(|_DL):/,"").trim();var d=item.d;if(!sym||!d)return;
      ...
      var sc=(f1?1:0)+(f2?1:0)+(f3?1:0)+(f4?1:0);STM[sym]=(sc===4||(f1&&sc===3))?1:-1;
    });
    buildWL($("wls")?$("wls").value:"");renderPF();
  }catch(e){}
}

// ─── FİYAT TABLOSU ───────────────────────────────────────────────────────────
function renderF(){
  var tablo=$("ftable");if(!tablo)return;
  var rows=(API.length>0)?API.map(function(r){return{sym:r.sym,rec:REC[r.sym],chg:CHG[r.sym]!=null?CHG[r.sym]:null};}):SL.map(function(s){return{sym:s.s,rec:REC[s.s],chg:CHG[s.s]!=null?CHG[s.s]:null};});
  rows.sort(function(a,b){var av=a.chg!=null?a.chg:-9999,bv=b.chg!=null?b.chg:-9999;return SD?(bv-av):(av-bv);});
  var parts=[];
  rows.forEach(function(r){
    var r2=rl(r.rec);var cs=r.chg!=null?(r.chg>=0?"+":"")+r.chg.toFixed(2)+"%":"—";var cc=r.chg!=null?(r.chg>0?"#22c55e":r.chg<0?"#ef4444":"#94a3b8"):"#2a3548";var ic=r.sym===CUR;
  ...
  tablo.innerHTML=parts.join("");var st2=$("fst");if(st2)st2.textContent=rows.length+" hisse — "+new Date().toLocaleTimeString("tr-TR");
}

// ─── PORTFÖY ─────────────────────────────────────────────────────────────────
function spf(q){var box=$("pfsug");if(!box)return;if(!q){box.style.display="none";return;}q=q.toUpperCase();var m=SL.filter(function(s){return s.s.includes(q)||s.n.toUpperCase().includes(q);}).slice(0,8);if(!m.length){box.style.display="none";return;}box.style.display="block";box.innerHTML=m.map(function(s){return'<div onclick="selPF(\''+s.s+'\',\''+s.n+'\')">'+s.s+' <span style="color:#4b5e78;font-size:10px;">'+s.n+'</span></div>';}).join("");}
function selPF(sym,nm){PFS=sym;var lbl=$("pflbl");if(lbl)lbl.textContent="✅ "+sym+" — "+nm;var sug=$("pfsug");if(sug)sug.style.display="none";var inp=$("pfsi");if(inp)inp.value=sym;}
function pfAdd(){var sym=PFS;if(!sym){alert("Önce hisse seç!");return;}if(PF.find(function(p){return p.sym===sym;})){alert("Zaten portföyde!");return;}PF.push({id:Date.now(),sym:sym});savePF();renderPF();}
function pfRem(){var sym=PFS;if(!sym){alert("Önce hisse seç!");return;}PF=PF.filter(function(p){return p.sym!==sym;});PFS="";savePF();renderPF();}
function pfDel(id){PF=PF.filter(function(p){return p.id!==id;});savePF();renderPF();}
function savePF(){try{localStorage.setItem("bist_pf",JSON.stringify(PF));}catch(e){}}
function loadPF(){try{var j=localStorage.getItem("bist_pf");if(j)PF=JSON.parse(j);}catch(e){}}
function renderPF(){var el=$("pflist");if(!el)return;if(!PF.length){el.innerHTML='<div style="padding:16px;font-size:11px;color:#2a3548;text-align:center;">Portföy boş.</div>';return;}var html="";PF.forEach(function(p){var r2=rl(REC[p.sym]);var chg=CHG[p.sym];var cs=chg!=null?(chg>=0?"+":"")+chg.toFixed(2)+"%":"—";var cc=chg!=null?(chg>0?"#22c55e":chg<0?"#ef4444":"#94a3b8"):"#2a3548";var nm="";for(var k=0;k<SL.length;k++)if(SL[k].s===p.sym){nm=SL[k].n;break;}html+='<div style="padding:10px 12px;border-bottom:1px solid #0f1520;display:flex;align-items:center;justify-content:space-between;"><div><div style="display:flex;align-items:center;gap:6px;margin-bottom:2px;"><span onclick="pick(\''+p.sym+'\')" style="font-size:14px;font-weight:700;color:#e8b84b;cursor:pointer;">'+p.sym+'</span><span style="font-size:9px;font-weight:700;padding:1px 6px;background:'+r2.bg+';color:'+r2.c+';">'+r2.t+'</span><span style="font-size:11px;font-weight:700;color:'+cc+';">'+cs+'</span></div><div style="font-size:10px;color:#3d5878;">'+nm+'</div></div><button onclick="pfDel('+p.id+')" style="background:#300;border:none;color:#f44;cursor:pointer;border-radius:3px;padding:3px 10px;font-size:12px;font-family:inherit;">✕</button></div>';});el.innerHTML=html;}

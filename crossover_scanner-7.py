"""
================================================================================
  Vortex-BIST · Supertrend × EMA(20, high) Crossover Scanner
  v1.2 — Hata düzeltmesi: EMA kaynağı CLOSE → HIGH (TV ile uyumlu)
        v1.1 yanlışlıkla CLOSE kullanıyordu, sahte yukarı-kesişim üretiyordu.

  Tarama Mantığı (günlük bar):
    - Yeşil çizgi  = Supertrend(ATR=2, factor=1.0)   ← TV: "Supertrend 2 1"
    - Sarı çizgi   = EMA(high, length=20)            ← TV: "EMA 20 high"
    - Koşul        = Supertrend > EMA(20,high)  (yukarı kesişim aktif)
...
# ============================================================================
#  GÖSTERGE PARAMETRELERİ — değiştirmek istersen burayı düzenle
# ============================================================================
ST_ATR_LENGTH = 2       # Supertrend ATR period (TV: "Supertrend 2 1" → 2)
ST_FACTOR     = 1.0     # Supertrend multiplier (TV: "Supertrend 2 1" → 1)
EMA_LENGTH    = 20      # EMA period
EMA_SOURCE    = "high"  # EMA kaynağı: "high" (TV ile aynı) veya "close"
#                       # KRİTİK: TV'de "EMA 20 high" yazıyor → "high" olmalı.
#                       # Yanlış kaynak (close) sahte yukarı-kesişim üretir
#                       # (EMA daha düşük çıkar, ST onu kolayca "geçer").
...
def compute_ema(prices: pd.Series, length: int = 20) -> pd.Series:
    """
    Standart üstel hareketli ortalama.
    EMA[i] = (close[i] * k) + (EMA[i-1] * (1-k)),  k = 2/(length+1)
    pandas.ewm() Wilder değil, klasik EMA için adjust=False kullanılır.
    """
...
def _analyze_one(symbol: str, df: pd.DataFrame):
    ...
    try:
        ...
        if len(idx) < 60:                        # EMA(20)+Supertrend için yeterli geçmiş
            return None
        ...
        st, _direc = compute_supertrend(high, low, close,
        ...
        # ============================================================
        # SIKI KONTROL — gerçek bullish durum için çoklu savunma
        # ENTRA/TUKAS/KFEIN/DMSAS gibi yfinance-TradingView fark
        # vakalarını ele.
        # ============================================================
        BULL_BARS    = 5      # Son 5 günün HEPSİ ST > EMA(20) olmalı
        MIN_MARGIN   = 0.005  # Son barda en az %0.5 margin
        FRESH_DAYS   = 4      # Son bar tarihi 4 günden eski ise atla

        if len(st) < BULL_BARS or len(ema) < BULL_BARS:
            return None
        bull_window = (st.iloc[-BULL_BARS:] - ema.iloc[-BULL_BARS:])
        if bull_window.isna().any():
            return None
        if (bull_window <= 0).any():
            return None
        ...
        if last_ema <= 0:
            return None
        if (last_st - last_ema) / last_ema < MIN_MARGIN:
            return None

        # Veri tazeliği — yfinance gecikmesini yakala
        try:
        ...
        # Günlük % değişim: son iki bar'ın close fiyatları arasındaki fark
        daily_pct = 0.0
        try:
            if len(close) >= 2:
...
<title>🟢 Supertrend × EMA(20) · Vortex-BIST</title>
<style>
...
<div class="foot">Vortex-BIST · Supertrend(2,1) × EMA(high,20) · v1.2</div>

...
VOL_BASE_LONG = 60 # uzun hacim tabanı — haftalardır yükselen hissede 5/20 şişer; 5/60 gerçeği gösterir
...
VGATE_CENTER = 1.2;  VGATE_K = 5.0     # taze sıçrama kapısı (5/20)
VGATE_LONG_CENTER = 1.5; VGATE_LONG_K = 4.0   # sürdürülen katılım kapısı (5/60)
...
# Ortak veri yolu: main.py'nin fetch_ohlc'si (önbellek + yeniden deneme).
# install_bvi(app, fetch_ohlc) ile bağlanır; yoksa doğrudan yfinance'e düşer.
_SHARED = {"fetch": None}


def _normalize(df):
    """Kolon adlarını Büyük harfe çevirir (fetch_ohlc küçük döndürebilir)."""
    if df is None:
        return None
    esle = {}
    for c in df.columns:
    ...
    return df.rename(columns=esle) if esle else df


def _fetch_daily(symbol):
    # 1) ortak yol (önbellekli, dayanıklı)
    f = _SHARED.get("fetch")
    if f is not None:
        try:
            df = f(symbol, period=PERIOD)
            df = _normalize(_flatten(df)) if df is not None else None
...
def score_bvi(symbol, bkm=None, nvs=None):
    ...
    # ortak veri yolu varsa yfinance şart değil
    if pd is None or np is None or (yf is None and _SHARED.get("fetch") is None):
        return {"symbol": symbol, "hata": "veri kaynagi yok"}
    ...
    # ── DEĞİŞKENLER ────────────────────────────────────────────
    # V: hacim çarpanı — İKİ tabanlı (taze sıçrama VE sürdürülen katılım)
    v_fast = float(volume.iloc[-VOL_FAST:].mean())
    v_slow = float(volume.iloc[-VOL_SLOW:].mean())
    v_base = float(volume.iloc[-VOL_BASE_LONG:].mean()) if len(volume) >= VOL_BASE_LONG else v_slow
    V = v_fast / v_slow if v_slow > 0 else 0.0        # 5/20 (taze sıçrama)
    V_long = v_fast / v_base if v_base > 0 else 0.0    # 5/60 (sürdürülen katılım)
    ...
    # ── KAPILAR ─────────────────────────────────────────────────
    # Hacim kapısı: taze sıçrama (5/20) VEYA sürdürülen katılım (5/60).
    # Haftalardır yükselen hissede 5/20≈1 olur (taban şişmiştir) → 5/60 kurtarır.
    Vgate = max(_logistic(V, VGATE_CENTER, VGATE_K),
                _logistic(V_long, VGATE_LONG_CENTER, VGATE_LONG_K))
    ...
    # ── KARAR — teşhis ayrımı ───────────────────────────────────
    # Düşük/orta BVI ÇOK FARKLI sebeplerden olabilir; aynı "TUZAK" demek yanlış:
    #   • UZAMIŞ  : momentum GÜÇLÜ + kalite YÜKSEK ama fiyat aşırı uzamış (SANEL tepede) → kovalama
    #   • TUZAK   : momentum aşağı/yatay (KNFRT) → gerçek tuzak
    #   • HACİMSİZ: kırılım hacimle onaylanmamış
    strong_mom = Mgate >= 0.70
    high_quality = base_quality >= 60.0
    extended = ExtPen >= 0.25
    mom_dead = Mgate < 0.35

    if strong_mom and high_quality and extended:
        karar = "🔺 GÜÇLÜ TREND AMA ÇOK UZAMIŞ — kovalama; geri çekilmede gir"
        renk = "#e8b84b"; kategori = "uzamis"; cls = "bekle"
    elif BVI >= 55:
        karar = "🚀 KIRILIM DOĞRULANDI — tavan/agresif aday"
        renk = "#22c55e"; kategori = "dogrulandi"; cls = "al"
    elif BVI >= 30:
        karar = "⚠️ ŞÜPHELİ — zayıf onay, küçük/izle"
        renk = "#e8b84b"; kategori = "supheli"; cls = "bekle"
    elif mom_dead:
        karar = "❌ TUZAK — momentum aşağı/yatay (PAS)"
        renk = "#ef4444"; kategori = "tuzak"; cls = "alma"
    elif Vgate < 0.40:
        karar = "❌ HACİMSİZ — kırılım onaysız (PAS)"
        renk = "#ef4444"; kategori = "hacimsiz"; cls = "alma"
    else:
        karar = "❌ ZAYIF / YATAY — PAS"
        renk = "#ef4444"; kategori = "zayif"; cls = "alma"
...
def install_bvi(app, fetch_ohlc=None) -> None:
    """fetch_ohlc verilirse BVI ortak veri yolunu kullanır (önerilen)."""
    if fetch_ohlc is not None:
        _SHARED["fetch"] = fetch_ohlc
    if _installed["on"]:
        return
    _installed["on"] = True

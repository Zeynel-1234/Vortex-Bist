...
# ═══════════════════════════════════════════════════════════════
# EK DÜZELTME · 6. KAPI (BVI) + OTONOM ZAMANLAYICI
# ───────────────────────────────────────────────────────────────
# install_bvi: /bvi/{symbol} endpoint'i (Kapı 6 — Kırılım Doğrulama)
#   daha önce main.py'a hiç bağlanmamıştı → panelde Kapı 6 hata veriyordu.
# start_scheduler: her iş günü 18:30 TR'de /scan + ÖĞREN + /rs/refresh
#   otomatik çalışır → restart sonrası RS verisi tekrar dolar.
#   (AUTO_RUN_OFF=1 ile kapatılabilir.)
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# SAT KOMBO LAB · Stoch RSI × Chande Kroll "tepede sat" eş-güdüm motoru
#   /satlab/optimize       → ızgara taraması (train/test), en iyi parametreler
#   /satlab/check/{symbol} → tek hisse anlık eş-güdüm sat durumu
# ═══════════════════════════════════════════════════════════════
from sat_combo_lab import install_sat_combo
install_sat_combo(app, fetch_ohlc)

# ═══════════════════════════════════════════════════════════════
# FAKTÖR MOTORU · 1-3 aylık çok-faktörlü kompozit (akademik temelli)
#   /factor/status · /factor/refresh/run · /factor/{symbol}
#   NVS panelinde "FAKTÖR SKORU (1-3 AY)" kapısı bunu kullanır.
# ═══════════════════════════════════════════════════════════════
from factor_engine import install_factor
install_factor(app, fetch_ohlc)

...
from lab_store import (save_dna, load_dna, is_cached, delete_dna,
                       list_all_dna, storage_info)  # ← YENİ

app = FastAPI(title="Fraktal Kahin", version="2.3.0")
...
# LAB_CACHE artık RAM değil, DISK üzerinden (lab_store)
...
def fetch_ohlc(symbol: str, period: str = "1y") -> Optional[pd.DataFrame]:
    """
    BIST hissesi için OHLC verisi çek.
    
    AKILLI RETRY: Birincil ".IS" suffix başarısız olursa
    alternatif suffix'leri dene (KOZAL gibi hisseler için kurtarma).
    """
    symbol = symbol.upper().replace('.IS', '').strip()

    # Deneme sırası — ".IS" genelde çalışır, diğerleri fallback
...
def analyze_one(symbol: str, use_cache: bool = True) -> Dict:
    ...
    if df is None:
        result = {"sembol": symbol, "hata": "Veri çekilemedi (yfinance — tüm suffix'ler denendi)",
...
# ═══════════════════════════════════════════════════════════════════
# LAB — DNA Üretimi + KALICI SAKLAMA (v2.3 GÜNCELLENDİ)
# ═══════════════════════════════════════════════════════════════════
@app.get("/lab/{symbol}")
def lab_endpoint(symbol: str, force: bool = Query(False)):
    ...
    YENİ (v2.3):
      • DNA kartı DİSKE yazılır — Railway restart'ta silinmez
      • Akıllı yfinance retry — KOZAL gibi hisseler kurtarılır
      • Önce disk cache kontrol edilir (7 gün TTL)
    """
    symbol = symbol.upper().replace('.IS', '').strip()
    if not symbol.isalnum() or len(symbol) > 8:
        raise HTTPException(400, "Geçersiz sembol formatı")

    # Disk cache kontrol (force=False ise)
    if not force:
        cached = load_dna(symbol)
        if cached is not None:
            return cached

    # yfinance ile veri çek (akıllı retry ile)
    df = fetch_ohlc(symbol, period="10y")
    if df is None or len(df) < 2000:
        df = fetch_ohlc(symbol, period="max")
        if df is None:
            result = {'sembol': symbol, 'status': 'FAIL',
    ...
    # DİSKE KAYDET (başarılı sonuçları ve zayıfları da)
...
def lab_test_endpoint(symbol: str):
    """
    DNA kartının özeti.
    Artık DİSKTEN okur (RAM cache yerine).
    """
    symbol = symbol.upper().replace('.IS', '').strip()
    dna = load_dna(symbol)
...
# ═══════════════════════════════════════════════════════════════════
# LAB — LİSTELEME VE DEPOLAMA ENDPOİNTLERİ (v2.3 YENİ)
# ═══════════════════════════════════════════════════════════════════
@app.get("/lab_list")
def lab_list_endpoint(
    ...
    """
    Tüm saklanan DNA kartlarının özet listesi.
    Filtreler:
      • status — sadece belirli statüdeki hisseler
      • min_quality — minimum kalite eşiği
    """
...
def lab_storage_endpoint():
    """Depolama durumu — Railway volume çalışıyor mu?"""
    return storage_info()


@app.delete("/lab/{symbol}")
def lab_delete_endpoint(symbol: str):
    """Belirli bir hissenin DNA kartını sil (yeniden üretim için)."""
    symbol = symbol.upper().replace('.IS', '').strip()

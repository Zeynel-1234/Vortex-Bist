...
# ═══════════════════════════════════════════════════════════════════════════
# URİS SAT v2.2 — TAM TEPE BAR MOTORU ENDPOINT'LERİ
# ═══════════════════════════════════════════════════════════════════════════

# ── ENDPOINT 0: Kurulum doğrulama ──────────────────────────────────────
@app.get("/sell_v2_check")
def sell_v2_check_endpoint():
    """SAT v2 kurulum doğrulama."""
    result = {'side': 'SELL_V2', 'version': 'v2.2_clean', 'checks': []}
    try:
        from lab_sell_v2 import build_sell_v2_dna, detect_v2_sell_signals
        result['checks'].append({'name': 'lab_sell_v2 import', 'status': 'OK'})
    except Exception as e:
        result['checks'].append({'name': 'lab_sell_v2 import', 'status': 'FAIL',
                                  'error': str(e)[:200]})
        return result
    try:
        info = sell_v2_storage_info()
        result['checks'].append({
            'name': 'Storage', 'status': 'OK',
            'storage_dir': info.get('storage_dir'),
            'cached_dna_count': info.get('v2_dna_count')
        })
    except Exception as e:
        result['checks'].append({'name': 'Storage', 'status': 'FAIL',
                                  'error': str(e)[:200]})
    all_ok = all(c['status'] == 'OK' for c in result['checks'])
    result['overall'] = 'OK' if all_ok else 'PROBLEMS_DETECTED'
    return result
...
def sell_v2_endpoint(symbol: str, force: bool = Query(False)):
    """
    SAT v2 — 4 KAPILI BÜTÜNCÜL KARAR (AL'ın simetriği)
    
    Bu endpoint:
      1. SAT v2 DNA üretir (veya cache'den alır)
      2. NVS skoru çeker (cache'den hızlı)
      3. Fraktal skoru çeker
      4. Momentum durumunu çeker
      5. 4 kapılı SAT kararını verir:
         - Kapı 1: AL koşulu bozulmuş (NVS<65 VEYA Fraktal<65)
         - Kapı 2: Fiyat zirve bölgesinde
         - Kapı 3: SAT v2 DNA güvenilir (kalite ≥ 25, n_test ≥ 2)
         - Kapı 4: Momentum YUKARI_GÜÇLÜ DEĞİL
      6. Hepsi geçerse → GÜÇLÜ SAT, aksi takdirde → BEKLE
    """
    from lab_sell_v2 import current_sat_decision, get_current_resistance_status
    symbol = symbol.upper().replace('.IS', '').strip()
    if not symbol.isalnum() or len(symbol) > 8:
        raise HTTPException(400, "Geçersiz sembol formatı")

    # ─── 1. DNA (cache veya yeni üret) ───────────────────────────────
    dna = None
    if not force:
        dna = load_sell_v2_dna(symbol)
    
    if dna is None:
        df = fetch_ohlc(symbol, period="max")
        if df is None:
            return {'sembol': symbol, 'karar': 'BEKLE',
                    'hata': 'yfinance veri çekemedi'}
        if len(df) < 500:
            return {'sembol': symbol, 'karar': 'BEKLE',
                    'hata': f'Yetersiz tarihsel veri: {len(df)} bar (min 500)'}
        if len(df) > 10000:
        ...
        try:
            ...
            return {'sembol': symbol, 'karar': 'BEKLE',
                    'hata': f'build hatası: {str(e)[:150]}'}
        dna = _json_safe(dna)
        dna['_bar_count'] = len(df)
        if dna.get('dna_status') == 'GUCLU':
            save_sell_v2_dna(symbol, dna, ttl_days=7)
    
    # Anlık karar için DataFrame gerekli (peak zone için)
    try:
        df_recent = fetch_ohlc(symbol, period="1y")
    except Exception:
        df_recent = None
    if df_recent is None or len(df_recent) < 90:
        df_recent = pd.DataFrame()
    
    # ─── 2. NVS skoru (cache'den) ────────────────────────────────────
    nvs_score = None
    try:
        nvs_result = nvs_endpoint(symbol, force=False)
        if isinstance(nvs_result, dict):
            nvs_score = nvs_result.get('nvs') or nvs_result.get('nvs_score')
    except Exception:
        pass
    ...
    fraktal_score = None
    try:
        fraktal_result = fraktal_endpoint(symbol)
        if isinstance(fraktal_result, dict):
            fraktal_score = fraktal_result.get('fraktal_skoru') or \
                           fraktal_result.get('skor') or \
                           fraktal_result.get('score')
    except Exception:
        pass
    ...
    momentum_r3 = None
    try:
        mom_result = momentum_endpoint(symbol, force=False)
        if isinstance(mom_result, dict):
            momentum_risk = mom_result.get('risk_skoru')
            momentum_yorum = mom_result.get('yorum')
            momentum_r3 = mom_result.get('r3_pct')
    except Exception:
        pass
    
    # ─── 5. 4 KAPILI KARAR ───────────────────────────────────────────
    dna_quality = dna.get('quality') if dna else None
    dna_test_n = (dna.get('test_perf') or {}).get('n_signals') if dna else None
    
    karar = current_sat_decision(
        df_recent if not df_recent.empty else pd.DataFrame({'close': [0]}),
        dna_quality=dna_quality,
        dna_test_n=dna_test_n,
        nvs_score=nvs_score,
        fraktal_score=fraktal_score,
        momentum_risk=momentum_risk,
        momentum_yorum=momentum_yorum,
        momentum_r3=momentum_r3
    )
    
    # ─── 5b. DİNAMİK DİRENÇ HATTI DURUMU (YENİ — KARSN tipi tepe tespiti) ─
    direnc_hatti = None
    try:
        if not df_recent.empty and len(df_recent) >= 60:
            direnc_hatti = get_current_resistance_status(
                df_recent, pivot_window=5, n_pivots=3, tolerance_pct=2.0
            )
    except Exception as e:
        direnc_hatti = {'cizgi_var': False, 'sebep': f'Hata: {str(e)[:80]}'}
    
    # ─── 6. BÜTÜNCÜL SONUÇ ───────────────────────────────────────────
    return _json_safe({
        'sembol': symbol,
        'karar': karar.get('karar'),
        'aciklama': karar.get('aciklama'),
        'kapilar': [
            {'no': 1, 'isim': 'AL koşulu bozulmuş',
             'gecti': karar.get('kapi1'), 'detay': karar.get('kapi1_detay')},
            {'no': 2, 'isim': 'Fiyat zirve bölgesinde',
             'gecti': karar.get('kapi2'), 'detay': karar.get('kapi2_detay')},
            {'no': 3, 'isim': 'SAT v2 DNA güvenilir',
             'gecti': karar.get('kapi3'), 'detay': karar.get('kapi3_detay')},
            {'no': 4, 'isim': 'Momentum aşağı/yatay',
             'gecti': karar.get('kapi4'), 'detay': karar.get('kapi4_detay')},
        ],
        'gecen_kapi': sum(1 for k in [karar.get('kapi1'), karar.get('kapi2'),
                                       karar.get('kapi3'), karar.get('kapi4')]
                          if k is True),
        'dna': {
            'status': dna.get('dna_status') if dna else None,
            'mode': dna.get('mode') if dna else None,
            'quality': dna.get('quality') if dna else None,
            'reason': dna.get('dna_reason') if dna else None,
            'train_perf': dna.get('train_perf') if dna else None,
            'test_perf': dna.get('test_perf') if dna else None,
            'build_time_sec': dna.get('build_time_sec') if dna else None,
            'bar_count': dna.get('_bar_count') if dna else None,
        },
        'direnc_hatti': direnc_hatti,   # ← YENİ: KARSN benzeri dinamik direnç tespiti
        'girdiler': karar.get('girdiler')
    })


# ── ENDPOINT 2: Bir hissenin son N gündeki sinyalleri ──────────────────
@app.get("/sell_v2_today/{symbol}")
def sell_v2_today_endpoint(symbol: str, lookback_days: int = Query(15, ge=5, le=60)):
    """Bir hisse için son N gündeki v2 sinyalleri."""
    symbol = symbol.upper().replace('.IS', '').strip()
    df = fetch_ohlc(symbol, period="6mo")
    ...
    try:
        ...
        return {'sembol': symbol, 'hata': f'signal hatası: {str(e)[:100]}'}
    ...
    for i, (date, row) in enumerate(zip(recent_dates, recent_sigs.itertuples())):
        if row.signal_strength >= 1:
            label = {1: 'ZAYIF', 3: 'GUCLU', 4: 'KESIN'}.get(row.signal_strength, 'NOTR')
            signals_list.append({
                'tarih': str(date.date()),
    ...
    return {
        'sembol': symbol, 'lookback_days': lookback_days,
        'sinyal_sayisi': len(signals_list), 'sinyaller': signals_list,
...
def sell_v2_list_endpoint():
    kayitlar = list_sell_v2_dna()
    return {'side': 'SELL_V2', 'toplam': len(kayitlar), 'kayitlar': kayitlar}
...
# ── ENDPOINT 5: SAT v2 DNA sil ─────────────────────────────────────────
@app.delete("/sell_v2/{symbol}")
def sell_v2_delete_endpoint(symbol: str):
    symbol = symbol.upper().replace('.IS', '').strip()
    clean = ''.join(c for c in symbol.upper() if c.isalnum())
    if os.path.exists('/data') and os.access('/data', os.W_OK):
        path = f'/data/dna_sell_v2_cards/{clean}.json'
    else:
        path = f'/tmp/dna_sell_v2_cards/{clean}.json'
    deleted = False
    try:
...
# ═══════════════════════════════════════════════════════════════
# EK MOTORLAR · SAVUNMACI KURULUM — modül eksikse uygulama ÇÖKMEZ,
# log'a "[main] X yuklenemedi" yazar.
# ═══════════════════════════════════════════════════════════════
try:
    from bvi_engine import install_bvi
    install_bvi(app)                       # Kapı 6 — Kırılım Doğrulama (BVI)
except Exception as _e:
    print("[main] BVI yuklenemedi:", repr(_e))

try:
    from auto_scheduler import start_scheduler
    start_scheduler()                      # 18:30 TR otonom /scan+ÖĞREN+/rs/refresh
except Exception as _e:
    print("[main] auto_scheduler yuklenemedi:", repr(_e))

try:
    from sat_combo_lab import install_sat_combo
    install_sat_combo(app, fetch_ohlc)     # /satlab/* — StochRSI×ChandeKroll tepe
except Exception as _e:
    print("[main] sat_combo_lab yuklenemedi:", repr(_e))

try:
    from factor_engine import install_factor
    install_factor(app, fetch_ohlc)        # /factor/* — 1-3 ay kompozit (Kapı 7)
except Exception as _e:
    print("[main] factor_engine yuklenemedi:", repr(_e))

try:
    from makro_trend_lab import install_makro
    install_makro(app, fetch_ohlc)         # /makro/* — trend başlangıcı + evren + birleşik
except Exception as _e:
    print("[main] makro_trend_lab yuklenemedi:", repr(_e))

try:
    from dip_dna import install_dipdna
    install_dipdna(app, fetch_ohlc)        # /dna/dip/{symbol}
except Exception as _e:
    print("[main] dip_dna yuklenemedi:", repr(_e))

try:
    from hakem import install_hakem
...
try:
    from kap_yon_sondaj2 import install_kap_yon_sondaj2
...
try:
    from kap_yon import install_kap_yon
    install_kap_yon(app)  # /kapyon/*
except Exception as _e:
    print("[main] kap_yon yuklenemedi:", repr(_e))

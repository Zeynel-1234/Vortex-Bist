...
def sell_v2_endpoint(symbol: str, force: bool = Query(False)):
    """v2 SAT DNA - period='max' ile maksimum tarihsel veri."""
    symbol = symbol.upper().replace('.IS', '').strip()
    if not symbol.isalnum() or len(symbol) > 8:
        raise HTTPException(400, "Geçersiz sembol formatı")
    ...
    df = fetch_ohlc(symbol, period="max")
    if df is None:
        return {'sembol': symbol, 'status': 'YETERSIZ',
                'reason': 'yfinance veri çekemedi'}
    if len(df) < 500:
        return {'sembol': symbol, 'status': 'YETERSIZ',
                'reason': f'Yetersiz tarihsel veri: {len(df)} bar (min 500)'}
    ...
    try:
        ...
        return {'sembol': symbol, 'status': 'YETERSIZ',
                'reason': f'build hatası: {str(e)[:150]}'}

    dna = _json_safe(dna)
    dna['_bar_count'] = len(df)

    if dna.get('status') in ('GUVENILIR', 'ORTA', 'ZAYIF', 'BELIRSIZ'):

...
def run_unified_scan(force: bool = False) -> Dict:
    ...
    try:
        ...
        alpha_scan = {"sonuclar": [], "regime": {"regime": "NEUTRAL"}}
    alpha_map = {r["sembol"]: r for r in alpha_scan.get("sonuclar", [])}
    regime = (alpha_scan.get("regime") or {}).get("regime", "NEUTRAL")
    ...
    for sym, nv in nvs_all.items():
        ...
        results.append({
            "sembol": sym,
            "birlesik": round(uni, 1),
            "nvs": nvs,
            "alpha": alpha_score,
            "alpha_z": a.get("composite_z") if a else None,
            "rank": None,  # aşağıda doldurulur
            "karar": dec["karar"],
            "renk": dec["renk"],
            "gate1": dec["gate1"],
            "gate2": dec["gate2"],
    ...
    payload = {
        ...
        "regime": alpha_scan.get("regime"),
        "n_total": len(results),
        "sonuclar": results,
    }
    payload = ae._clean(payload)
    with _LOCK:
        _CACHE["t"] = time.time()
        _CACHE["data"] = payload
    return payload

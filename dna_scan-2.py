...
def _gist_save() -> Dict:
    ...
    try:
        if DNASCAN_GIST_ID:
            r = requests.patch("https://api.github.com/gists/" + DNASCAN_GIST_ID,
...
def install_dna_scan(app, fetch_ohlc: Callable) -> None:
    ...
    @app.get("/dnascan/debug")
    def dnascan_debug():
        """Gist baglantisinin saglik raporu — hicbir sey degistirmez."""
        rapor = {
            "env_DNASCAN_GIST_ID": (DNASCAN_GIST_ID[:6] + "..." +
                                    DNASCAN_GIST_ID[-4:]) if DNASCAN_GIST_ID
                                   else "YOK",
            "env_id_uzunluk": len(DNASCAN_GIST_ID),
            "env_GITHUB_TOKEN": "var" if GITHUB_TOKEN else "YOK",
            "bellek_olay": len(_STATE.get("olaylar", [])),
        }
        hd = _gist_headers()
        if not (hd and DNASCAN_GIST_ID and requests):
            rapor["sonuc"] = "gist erisimi kurulamiyor (env eksik)"
            return rapor
        try:
            r = requests.get("https://api.github.com/gists/" + DNASCAN_GIST_ID,
                             headers=hd, timeout=90)
            rapor["gist_get_kod"] = r.status_code
            if r.status_code == 200:
                files = r.json().get("files", {})
                rapor["dosyalar"] = list(files.keys())
                if GIST_DOSYA in files:
                    f = files[GIST_DOSYA]
                    rapor["kirpilmis"] = bool(f.get("truncated"))
                    rapor["boyut_byte"] = f.get("size")
                    try:
                        if f.get("truncated") and f.get("raw_url"):
                            rr = requests.get(f["raw_url"], headers=hd,
                                              timeout=120)
                            rapor["raw_kod"] = rr.status_code
                            d = json.loads(rr.text)
                        else:
                            d = json.loads(f["content"])
                        rapor["parse"] = "OK"
                        rapor["gist_olay"] = len(d.get("olaylar", []))
                        rapor["gist_tamam"] = d.get("tamam")
                    except Exception as e:
                        rapor["parse"] = "HATA: " + repr(e)[:120]
        except Exception as e:
            rapor["gist_get"] = "HATA: " + repr(e)[:120]
        return rapor

    print("[dna_scan] kuruldu: /dnascan/start · /dnascan/run · "
          "/dnascan/status · /dnascan/ozet · /dnascan/retry · /dnascan/debug")

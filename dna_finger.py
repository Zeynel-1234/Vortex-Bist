...
def _gist_oku(gist_id, dosya):
    ...
    try:
        r = requests.get("https://api.github.com/gists/" + gist_id,
                         headers=hd, timeout=30)
        if r.status_code == 200:
            files = r.json().get("files", {})
            if dosya in files:
                f = files[dosya]
                if f.get("truncated") and f.get("raw_url"):
                    rr = requests.get(f["raw_url"], timeout=60)
...
def _tek_hisse(sym: str, olaylar: List[Dict], fetch_ohlc: Callable) -> Dict:
    df = None
    try:
    ...
    for o in olaylar:
        ...
        if i is None:
            continue
        kay = {"s": sym, "t0": o["t0"], "sf": o["sf"], "mg": o.get("mg")}
        gecerli = True
        for k in OZELLIKLER:
            val = diz[k][i]
    ...
    def dnafinger_start():
        ...
        if not scan or not scan.get("olaylar"):
            return {"hata": "dna_scan havuzu okunamadi",
                    "ipucu": "DNASCAN_GIST_ID ortam degiskeni dogru mu?"}
        ...
        g = _gist_save()
        return {"durum": "basladi", "hisse": len(gruplar),
